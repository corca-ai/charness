...
For a repair after a failed, blocked, deferred, timed-out, or partially
delivered review, reuse its carrier instead of reopening the original packet:

```bash
python3 "$SKILL_DIR/scripts/run_review.py" \
  --repo-root . \
  --follow-up-from charness-artifacts/critique/workers/<attempt>/partial-result.json \
  --scope "repair of the prior bounded finding" \
  --lens "recheck the finding and its final consumer" \
  --dry-run
```

Without `--reviewed-path`, this derives the smallest current delta from the
prior working-tree packet: only previously reviewed paths whose bound bytes
changed are included. Use repeatable `--reviewed-path` values when the repair
adds a new file, changes an unreviewed consumer, or needs a deliberate wider
stimulus. The prior packet is canonically verified, its recorded identity is
self-verified, and its retained semantic-input carriers are checked against
the identity before selection; a missing or tampered carrier refuses reuse.
For committed-ref reviews, implicit reuse also requires the same pinned ref.
Explicit paths carry a selection receipt describing changed, new, unchanged,
and omitted prior paths. The follow-up packet carries a bounded,
content-addressed summary of the prior findings as inert context; the final
packet identity is joined after packet assembly, and the context has a hard
24 KiB bound. It never reuses the prior verdict. A follow-up still needs its
own packet identity, worker delivery, and approval readback.
...
the source paths that establish every target's claims. The labels stay opaque
membership data for the consuming skill; they never auto-declare a coverage
floor. To enforce per-target coverage on a lane run, repeat `--expected-target
<target>` on `run_review.py` — usually once per prepared target — and the lane
instructs the prompt to emit exactly one `target_observations` entry per
declared target: the floor then refuses a schema-valid result that silently
drops one as incomplete coverage. Direct `run_reviewer_worker.py` invocations
declare through its own `--expected-target` flag. Adding labels does not
expand what was reviewed. Existing packets cannot be combined with
...
across runs. Follow-up reuse is deliberately outside producer caching: it
narrows the new identity-bound input while preserving the old carrier as
evidence, so a stale or mutated old result cannot silently become approval.
...
Follow-up packets add one optional adapter contract, `follow_up_scope`: one
entry per `packet_sections` item with `mode: selected-paths` or `mode: static`.
Selected-path producers must expose an exact reviewed-path manifest in their
section body and declare `path_binding: exact-path-manifest`; extras and
omissions are refused, and the receipt records a path-set digest. Static
sections must explicitly declare that they are not path-bearing. The runner
writes and the consumer independently rechecks a verified producer-scope
receipt. This is a scope proof, not approval and not a producer cache.

Retained recovery is fail-closed. A report, receipt, delivery ledger, result,
runner stream, and attempt id must all join to the retained run before
promotion. Invalid recovery emits a typed `status: refused` result and retains
the scratch evidence for diagnosis or retry; only verified promotion may remove
