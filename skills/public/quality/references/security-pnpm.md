...
`<authoring-repo>/scripts/check_supply_chain.py` enforces the root-level alignment
...
- run pnpm's audit flow only where the team is prepared to triage advisory
  ...
  `<authoring-repo>/scripts/check_supply_chain_online.py` with `pnpm audit --json`
