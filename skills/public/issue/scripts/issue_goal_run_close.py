...
def existing_close_result(
    ...
    if "already_closed" in prepared:
        if CONTRACT.CLOSE_CONTRACT.prior_adjudication_review(prepared) != expected:
            raise RuntimeError(
...
def command_close(args: Any, *, resolve_backend: Any, emit: Any) -> int:
    ...
    try:
        ...
        return 2
    try:
        existing_close = existing_close_result(
    ...
    if prepared.get("recovery"):
        ...
        adjudication_review = proof["final_proof_index"]["adjudication_review"]
        if adjudication_review:
            result["parent_adjudication_review"] = adjudication_review
        try:
