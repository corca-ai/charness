...
def resumable_state(
    ...
    *,
    ...
    claims_boundaries: dict[str, str] = {}

    def _is_claims_evidence(commit: str) -> bool:
        if not tagged_prepared or not tagged_claims_evidence:
            return False
        boundary = claims_evidence_boundary(
            cli, repo_root, commit, tag_name=tag_name, record_path=record_path
        )
        claims_boundaries[commit] = boundary
        return boundary == tagged_claims_evidence
    ...
    if tag_sha and head_sha == tag_sha and tag_state["remote"] and release_exists:
        # Publication can succeed before the first closeout observer/install
        # step.  The tag itself is then the only durable local boundary; keep
        # it resumable instead of later misreporting the already-published
        # state as complete.
        phase = "post-publication-pending"
    elif tag_sha and parent_sha == tag_sha and close_refs:
    ...
    return {
        ...
        "claims_parent_boundary": claims_boundaries.get(parent_sha, parent_sha),
        "claims_grandparent_boundary": claims_boundaries.get(grandparent_sha, grandparent_sha),
