...
def _revalidate_claims_boundary(
    repo_root: Path, state: dict[str, Any], claims_review_validator: Any | None
) -> None:
    """Revalidate claims evidence immediately before a reconstructed publish."""
    if state["phase"] not in _CLAIMS_PHASES:
        return
    if not state.get("claims_review"):
        raise SystemExit(
    ...
    if claims_review_validator is None:
        raise SystemExit(
            "--resume: publication boundary has no claims-review validator; refusing "
            "to publish a reconstructed claims state without revalidation."
        )
    try:
        state["claims_review"] = claims_review_validator(repo_root, state)
    except (SystemExit, OSError, ValueError) as exc:
        if isinstance(exc, SystemExit):
            raise
        raise SystemExit(
            f"--resume: claims-review revalidation failed at the publication boundary: {exc}"
        ) from exc
    if not isinstance(state["claims_review"], dict) or state["claims_review"].get("verdict") not in {
        "pass",
        "unproven",
    }:
        raise SystemExit(
            "--resume: publication-bound claims review is not a validated pass or unproven record"
        )
...
def _promote_quality_receipt(
    ...
    *,
    ...
    """Make the one-push quality proof survive release scratch cleanup."""
    if owner is None or receipt_path is None or not receipt_path.is_file():
        return
    # A resume reruns the quality gate and seals a fresh receipt.  An earlier
    # attempt may already have left a path in the payload, but that path is not
    # proof that its bytes describe this attempt's head or materialized root.
    # Always replace the durable copy with the newly sealed receipt.
    destination = (
        repo_root / record_path
    ).parent / f"{tag_name.removeprefix('v')}-prepush-quality.json"
    durable_receipt = owner.promote_file(receipt_path, destination)
    payload["prepush_quality_receipt"] = str(destination.relative_to(repo_root))
    payload["prepush_quality_receipt_owner"] = str(durable_receipt.relative_to(repo_root))
    try:
        owner_record = json.loads(durable_receipt.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise SystemExit(f"release: promoted quality receipt owner record is unreadable: {durable_receipt}") from exc
    payload["prepush_quality_receipt_sha256"] = owner_record.get("evidence_sha256")
...
def resume_publish(repo_root: Path, *, args: Any, plan: dict[str, Any], adapter_data: dict[str, Any], cli: Any,
                   ...
                   resume_closeout: Any, commit_artifact_before_push, release_record_path,
                   claims_review_validator: Any | None = None) -> None:
    """Run resume publication with an owned receipt scratch lifecycle."""
    if not args.execute:
        return _resume_publish_impl(
            repo_root,
            args=args,
            plan=plan,
            ...
            common=common,
            resume_closeout=resume_closeout,
            commit_artifact_before_push=commit_artifact_before_push,
            release_record_path=release_record_path,
            receipt_path=None,
            claims_review_validator=claims_review_validator,
        )
    owner = owned_scratch(repo_root, "release-prepush-quality")
    receipt_path = owner.open() / "receipt.json"
    owner_state = "failed"
    try:
        result = _resume_publish_impl(
            repo_root,
            args=args,
            plan=plan,
            adapter_data=adapter_data,
        ...
        # The receipt is normally promoted immediately after the gate. This
        # fallback preserves it too when a later step fails before publication.
        try:
            try:
                _promote_quality_receipt(
                    repo_root,
                    owner=owner,
                    receipt_path=receipt_path,
                ...
                # A durable file may have been copied before its sidecar was
                # verified.  Explicitly downgrade the owner back to retained
                # evidence so close() cannot delete the only retry source.
                try:
                    owner.retain_failed_promotion(
                        recovery_reason="quality receipt promotion/read-back failed",
            ...
            owner.close(state=owner_state)


def _resume_publish_impl(repo_root: Path, *, args: Any, plan: dict[str, Any], adapter_data: dict[str, Any], cli: Any,
                   state: dict[str, Any] | None, resumable_state, assert_resumable, common: Any,
                   resume_closeout: Any, commit_artifact_before_push, release_record_path,
                   receipt_path: Path | None, receipt_owner: Any | None = None,
                   claims_review_validator: Any | None = None) -> None:
    ...
    _revalidate_claims_boundary(repo_root, state, claims_review_validator)
