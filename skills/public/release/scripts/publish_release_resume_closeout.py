...
def resume_post_publication_closeout(
    ...
    *,
    ...
    if state["phase"] == "post-publication-pending":
        # The first post-publication closeout step may fail while HEAD is still
        # exactly the published tag.  There is no issue-close carrier to
        # validate yet, but the external release is real and the remaining
        # closeout is safe to retry idempotently from this boundary.
        if args.close_issue:
        ...
        if not args.execute:
            payload["resume"] = "dry-run: would continue pending post-publication closeout"
            emit_yaml(payload)
            return
        ...
        continued = {
            **state,
            "head_sha": cli.run(["git", "rev-parse", "HEAD"], cwd=repo_root).stdout.strip(),
            "remote_branch_sha": state.get("remote_branch_sha", ""),
        }
        _reconcile_push(
            repo_root,
            state=continued,
        ...
        payload["resume"] = "pending post-publication closeout completed"
        emit_yaml(payload)
        return
    _require_closeout_resume_inputs(args)
    common.preflight_close_issue_carrier(
