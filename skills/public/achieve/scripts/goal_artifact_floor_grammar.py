...
The deterministic closeout floors (backlog, operator-queue, blocked-matrix,
disposition, figure-form, and evidence-distinctness) share the same parse
primitives:
...
def section_span(masked: str, heading: str) -> tuple[int, int] | None:
    ...
    the body. The disposition and figure-form floors use this variant; the
    operator-queue and blocked-matrix floors keep their own flat ``## ``-only
    variant because their sections are intentionally exact.
    """
    start = re.compile(
...
def joined_section_body(text: str, heading: str) -> str | None:
    ...
    Backlog rows may wrap across physical lines. Joining first keeps a logical
    row together while preserving ``None`` for an absent section.
    """
