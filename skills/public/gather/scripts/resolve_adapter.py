...
ADAPTER_CANDIDATES = (
    ...
    Path(".codex/gather-adapter.yaml"),
    Path(".claude/gather-adapter.yaml"),
    Path("docs/gather-adapter.yaml"),
    Path("gather-adapter.yaml"),
)
...
def load_adapter(repo_root: Path) -> dict[str, Any]:
    ...
    warnings: list[str] = []
    canonical_path = repo_root / ".agents" / "gather-adapter.yaml"
    if not isinstance(raw, dict):
...
def main() -> None:
    cancel_timeout = SKILL_RUNTIME.arm_cli_timeout(label="gather resolve_adapter")
    parser = argparse.ArgumentParser()
    parser.add_argument("--repo-root", type=Path, required=True, help="Repo root whose gather adapter should be resolved")
    try:
