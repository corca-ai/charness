...
def build_items(repo_name: str, _args: object) -> list[tuple[str, object]]:
    return base_adapter_items(repo_name, "charness-artifacts/debug")


def main() -> None:
    output = run_init_adapter(default_output=Path(".agents/debug-adapter.yaml"), build_items=build_items)
