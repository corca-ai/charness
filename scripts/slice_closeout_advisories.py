...
def is_artifact_only_commit(changed_paths: list[str]) -> bool:
    """A commit whose changed paths are ALL under ``charness-artifacts/`` —
    operational scratch (goal-shaping / handoff / retro / debug) with no
    code/test/skill/doc fix surface. One shared definition of "artifact-only" so
    the over-slicing advisory and the close-keyword-leakage advisory (#363) cannot
    drift apart. An empty path list is not artifact-only."""
    return bool(changed_paths) and all(path.startswith("charness-artifacts/") for path in changed_paths)
...
def trailing_artifact_only_run(commit_path_lists: list[list[str]]) -> int:
    ...
    for paths in commit_path_lists:
        if is_artifact_only_commit(paths):
            run += 1
        else:
            break
    return run
