...
def validate_materialized_plugin_export(
    ...
    *,
    ...
    # The byte comparison proves mirror parity; the consumer smoke proves that
    # the bytes import without the authoring checkout masking missing package
    # dependencies. Keep both in the packaging validator so a release cannot
    # claim an install surface that only passes the separate smoke command.
...
def validate_materialized_plugin_export_matches_generated(
    ...
    with owned_scratch(root, "plugin-export-validation") as tmpdir:
