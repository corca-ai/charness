...
try:
    ...
    OwnedScratch = _scratch.OwnedScratch
except ModuleNotFoundError:  # minimal quality fixtures omit the optional owner module
    class OwnedScratch:  # type: ignore[no-redef]
        """Compatibility owner for a deliberately partial copied runner fixture."""

        def __init__(self, repo_root: Path, producer: str, *, runtime_root_path: Path) -> None:
            del repo_root
            self._base = runtime_root_path / "tmp"
        ...
        def open(self) -> Path:
            ...
            self._directory = tempfile.TemporaryDirectory(
                prefix=f"{self._producer}-", dir=self._base
            )
            self.path = Path(self._directory.name)
            return self.path

        def close(self, *, state: str = "succeeded") -> None:
            del state
...
class RuntimeContext:
    ...
    temp_dir: Path
    regime: str
    scratch_owner: OwnedScratch | None = None
...
def close_runtime(context: RuntimeContext, *, state: str | None = None) -> None:
    if context.scratch_owner is not None:
        context.scratch_owner.close(state=state or context.terminal_state)
