...
def collect_offline_findings(repo_root: Path) -> list[str]:
    findings: list[str] = []
    findings.extend(validate_javascript_surface(repo_root))
    findings.extend(validate_python_surface(repo_root))
    return findings


def validate_javascript_surface(repo_root: Path) -> list[str]:
    findings: list[str] = []
...
def validate_python_surface(repo_root: Path) -> list[str]:
    findings: list[str] = []
...
def detect_online_audit_surfaces(repo_root: Path) -> list[dict[str, str]]:
    surfaces: list[dict[str, str]] = []
    js_findings = validate_javascript_surface(repo_root)
    for finding in js_findings:
        manager, _, lockfile = finding.partition(":")
        if manager in {"npm", "pnpm"} and lockfile:
            surfaces.append({"tool": manager, "lockfile": lockfile, "surface": "javascript"})

    pyproject = repo_root / "pyproject.toml"
    if pyproject.exists() and detect_declared_python_dependencies(pyproject):
        surfaces.append({"tool": "uv", "lockfile": "uv.lock", "surface": "python"})
    return surfaces
