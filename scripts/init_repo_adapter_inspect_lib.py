...
from scripts.init_repo_agent_docs_lib import _recommendation
...
INIT_REPO_ADAPTER_CANDIDATES: tuple[Path, ...] = (
...
def detect_init_repo_adapter_normalization(
    repo_root: Path,
) -> tuple[dict[str, object], list[dict[str, object]]]:
    """Promote init-repo-adapter absence from a warning to an advisory recommendation."""
    adapter_path: Path | None = next(
        (repo_root / candidate for candidate in INIT_REPO_ADAPTER_CANDIDATES if (repo_root / candidate).is_file()),
        None,
    )
    ...
    if not adapter_exists:
        recommendations.append(
            _recommendation(
                rec_id="init_repo_adapter_missing",
                target=INIT_REPO_ADAPTER_CANDIDATES[0].as_posix(),
                kind="seed_artifact",
                priority="advisory",
                confidence="high",
                enforcement_tier="NON_AUTOMATABLE",
                evidence=[
                    "No init-repo adapter found in any candidate path.",
                    "Using default surface paths and durable artifact location.",
                ],
                suggested_action=(
                    f"Create {INIT_REPO_ADAPTER_CANDIDATES[0].as_posix()} to record preset "
    ...
    return (
        {
            ...
            "candidates": [candidate.as_posix() for candidate in INIT_REPO_ADAPTER_CANDIDATES],
        },
        recommendations,
    )
