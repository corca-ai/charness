...
> not blocked.
>
> Named for the day it was WRITTEN and its subject, not for a sequence. See the
> record-integrity note at the end: the five plans before it are misdated.
...
## A record-integrity note, because this file nearly repeated it

The five `charness-artifacts/design-studies/*-next-session-plan.md` files dated
`2026-08-29` through `2026-09-02` were **all written on 2026-08-29**:

```
git log --diff-filter=A --format=%ad --date=short -1 -- <each file>
```

returns `2026-08-29` for every one. Someone began incrementing the date as if it
were a sequence number, and each following session — including this one, until
the operator asked why it had become September — copied the pattern instead of
reading it. The first draft of THIS file was named `2026-09-03`.

Nothing enforces the convention: design-studies has no surface contract, so no
gate refused any of them. But `check_spec_evidence_durability` grandfathers by
FILENAME date, so a filename date is load-bearing somewhere in this repo, and
four of these five claim to be from the future.

This is small and it is exactly the class this repo spends itself on: a record
that reads as a fact and is not one. Renaming the five is a five-minute change
that touches history, so it is a decision rather than a chore — but the next
session should not add a sixth.
