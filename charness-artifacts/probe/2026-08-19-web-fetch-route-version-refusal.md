...
Call sites unproven: none — `resolve_github_mode` holds the file's only adapter read, and
  the guard sits between the load and the first use of `gather_provider`. Its one
  production caller is `route_public_fetch.py:58`, which calls it rather than re-reading
  the adapter
...
raised as `SystemExit`, measured for both declared modes and for both doors
(`version: 9` and `version: !!int 9` — gather routes through `simple_skill_adapter_lib`,
so the parse door renders the rendered refusal rather than a traceback).
