...
| Markdown/doc | `check_doc_links`, `check-markdown`, `check_command_docs` | **keep** | Cheap, deterministic, distinct surfaces. **2026-08-09 correction:** #563 supersedes this row's former `check_title_slug_drift` keep call. That advisory had no recorded catch, excluded the live goal population, and reduced non-English titles to unusable tokens; it was deleted. Title/slug coherence now stays a bounded reviewer judgment over H1, filename, incoming links, generated indexes, and first-reader friction, with no aggregate clean verdict. |
