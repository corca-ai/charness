...
- Adding the new quality artifact changed the measured quality-artifact corpus. The first broad run exposed four stale recorded-measurement values, and the inventory-consumption test exposed that the artifact had not engaged the exact declared fields. This was useful detection, but it created avoidable rework because the quality artifact and its probe baselines were not treated as one sync unit at first. (source: `charness-artifacts/retro/2026-08-04-session-retro.md`; sources: 2)
