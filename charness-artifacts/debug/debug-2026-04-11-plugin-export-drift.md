...
`charness update` stopped at `managed checkout ... has local changes` instead of
reporting Codex cache drift.
...
- A test-style repro showed dirty files immediately after init.
...
Running `charness init` into a temp home, then `git status --short` inside
the managed checkout showed uncommitted changes in `plugins/charness/`.
