...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 330.3s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.2.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.2.0`
...
- Response content checked for: `v8.2.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.2.0`
- HTTP status: `200`
...
- Claims review record: `charness-artifacts/release-review/2026-09-04-v8.2.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: run_review.py attempt release-8-2-0-claims-1 delivered findings-received verdict pass; packet 518a44081a40dfa2f98bc9e6f0b15bdfe0bc7a1413a594cf788bf0300fca5071
- Review narrative: `charness-artifacts/release-review/2026-09-04-v8.2.0-claims-review.md`.
- Verdict scope: 43 blocking path(s) gated this tag; 2 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 8 defect(s) recorded in the advisory scope and SHIPPED KNOWN-INACCURATE rather than repaired before this tag:
  - CLAIM-001 medium: lane, timings, and probe results asserted by the helper without raw receipts; measured in this prepare and standalone
  - CLAIM-002 low: exact current_release.py result not bound
  - CLAIM-003 medium: adapter-unchanged reason not re-derivable from packet; measured empty diff v8.1.0..f4b85e906
  - CLAIM-004 medium: no-member-change claim not provable from bound surfaces; measured identical skill, adapter, and subcommand sets
  - CLAIM-005 medium: task-run runtime behaviour and 'in every record' proven by tests, not by the packet
  - CLAIM-006 medium: critique record is a narrative; the six reviewer packets are tracked with SHA256s
  - CLAIM-007 low: minor-bump reading and prepared Git state are authored judgment; policy is version-policy.md Minor
  - CLAIM-008 low: README corrected-against-init.sh claim exceeds the packet; corrections are commit b528a0add
...
- Public release check: `gh release view v8.2.0`.
...
- `cli_skill_surface_gate`: 2.202s
- `quality_command`: 330.276s
- `fresh_checkout_probes_resume`: 4.296s
- `push_create_verify_release`: 5.051s
- `distinct_channel_verification`: 0.709s
- `published_notes_audit`: 0.444s
- `post_publish_install_refresh`: 7.498s
- `post_publish_installed_readback`: 1.040s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-04-v8.2.0-release-observer.json`.
