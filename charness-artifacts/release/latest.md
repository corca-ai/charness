...
Advanced `charness` toward release `8.3.0` (tag `v8.3.0`) through the repo-owned release helper.
...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 330.0s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.3.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.3.0`
...
- Response content checked for: `v8.3.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.3.0`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/2026-09-04-worktree-dependency-reuse-792-release-8-3-0-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-04-v8.3.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: codex_exec worker attempt release-8-3-0-claims-2, packet sha256 79a9ab617c2061ca848883e1d04f6366b06ad246ec45adf07c35aab68999d717, delivery findings-received, verdict pass
- Review narrative: `charness-artifacts/release-review/2026-09-04-v8.3.0-claims-review.md`.
- Verdict scope: 57 blocking path(s) gated this tag; 4 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 5 defect(s) recorded in the advisory scope and SHIPPED KNOWN-INACCURATE rather than repaired before this tag:
  - CR2-A1: quality, gate, probe outcomes and runtime figures are helper assertions without bound receipts
  - CR2-A2: the four-plus-one release-surface census depends on current_release.py derivation not bound in the packet
  - CR2-A3: the minor rationale's change-relative claims lack an 8.2.0 baseline in the packet
  - CR2-A4: the #792 critique record's reviewer identities and test counts are attestations, their packets tracked separately
  - CR2-A5: previous version, branch, remote, and unchanged release adapter are not established by the packet
...
- Public release check: `gh release view v8.3.0`.
...
- `quality_command`: 330.039s
- `fresh_checkout_probes_resume`: 4.497s
- `push_create_verify_release`: 4.764s
- `distinct_channel_verification`: 0.486s
- `published_notes_audit`: 0.496s
- `post_publish_install_refresh`: 6.834s
- `post_publish_installed_readback`: 1.094s
- `release_observer`: 0.000s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-04-v8.3.0-release-observer.json`.
...
> minor, not patch: prepare.dependency_reuse is a new additive adapter field, --no-dependency-reuse a new flag, and dependency_reuse a new payload and result.json key; existing manifests are unaffected until they declare it, and nothing was renamed or removed.