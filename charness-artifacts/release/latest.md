...
Advanced `charness` toward release `8.4.0` (tag `v8.4.0`) through the repo-owned release helper.
...
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.4.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.4.0`
...
- Response content checked for: `v8.4.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.4.0`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/2026-09-04-release-8-4-0-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-04-v8.4.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: bounded-reviewer claims audit of latest.md at 85ede6996 delivered findings-received verdict pass; no record defect
- Review narrative: `charness-artifacts/release-review/2026-09-04-v8.4.0-claims-review.md`.
- Verdict scope: 58 blocking path(s) gated this tag; 5 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 5 defect(s) recorded in the advisory scope and SHIPPED KNOWN-INACCURATE rather than repaired before this tag:
  - CLAIM-001 medium: quality 350.1s/exit 0 is helper-measured; gitignored last-release-receipt corroborates pass but is not in the prepared commit
  - CLAIM-002 low: runtime figures exist only in the helper record
  - CLAIM-003 low: current_release.py was not re-executed; versions re-derived from packaging and marketplace files
  - CLAIM-004 medium: fresh-checkout passed has no raw receipt; listed commands match the adapter
  - CLAIM-005 low: adapter-unchanged was not git-diff'd against v8.3.0
...
- Public release check: `gh release view v8.4.0`.
...
- `cli_skill_surface_gate`: 2.302s
- `quality_command`: 189.874s
- `fresh_checkout_probes_resume`: 4.544s
- `push_create_verify_release`: 0.485s
- `distinct_channel_verification`: 0.500s
- `published_notes_audit`: 0.466s
- `post_publish_install_refresh`: 10.422s
- `post_publish_installed_readback`: 1.066s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-04-v8.4.0-release-observer.json`.
...
> minor, not patch: --ephemeral/--owned, create-time reclaim, and unlabeled throwaway reclaim are new operator-facing behavior; existing ../feature-worktree callers do not migrate.