...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 350.1s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh --release --read-only`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.4.0`, checked at `post-bump, pre-commit`.
...
- `cli_skill_surface_gate`: 2.175s
- `quality_command`: 350.059s
- `fresh_checkout_probes_initial`: 4.619s
