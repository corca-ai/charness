...
Advanced `charness` toward release `8.7.2` (tag `v8.7.2`) through the repo-owned release helper.

## Current Version

- previous version: `8.7.1`
- target version: `8.7.2`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 255.7s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/3716570-1789446589878448135/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.7.2-prepush-quality.json` (sha256: `e72353a63afe372b8eef916da905752bc5c6888ff3c6083f64a17c0251b6230f`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.7.2`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.7.2`
...
- Response content checked for: `v8.7.2`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.7.2`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v872-release-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-15-v8.7.2-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: workflow child agent claims review over the new prepared 8.7.2 record (recovered line): verdict pass, findings empty, 9 path:line-backed evidence items covering version bump, prepare diff shape, state marker, quality-line honesty, probe claims, pending-state honesty, bump rationale, and critique binding; unresolved empty
- Review narrative: `charness-artifacts/release-review/2026-09-15-v8.7.2-claims.md`.
- Verdict scope: 201 blocking path(s) gated this tag; 2 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.7.2`.
...
- `cli_skill_surface_gate`: 2.190s
- `quality_command`: 255.664s
- `fresh_checkout_probes_resume`: 5.427s
- `push_create_verify_release`: 269.436s
- `distinct_channel_verification`: 0.594s
- `published_notes_audit`: 0.445s
- `post_publish_install_refresh`: 7.477s
...
- Durable observer record: `charness-artifacts/probe/2026-09-15-v8.7.2-release-observer.json`.
