...
Advanced `charness` toward release `8.14.1` (tag `v8.14.1`) through the repo-owned release helper.

## Current Version

- previous version: `8.14.0`
- target version: `8.14.1`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 613.3s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/615174-1790395347024083154/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.14.1-prepush-quality.json` (sha256: `8c058231cc5e471b94893144428df37af515a6b6a681eb412acdf496a8697cc0`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.14.1`, checked at `post-claims-review, pre-push`.
- Validated tree: commit `488ea8fbbe50d1e66d7aaed9857b9d9771b0822f` (tree `0899404f3d84d9635e04d020d6f52e341c68a967`); a pushed tag or branch that does not contain this tree was not what this check validated.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.14.1`
...
- Response content checked for: `v8.14.1`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.14.1`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v8141-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-26-v8.14.1-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: fresh subagent task v8.14.1-claims-r2 rendered R2-PASS citing latest.md:21, file:line prepare ordering, and git tree comparison, independent of the round-1 reviewer
- Review narrative: `charness-artifacts/release-review/2026-09-26-v8.14.1-claims-review.md`.
- Verdict scope: 35 blocking path(s) gated this tag; 0 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.14.1`.
...
- `cli_skill_surface_gate`: 2.440s
- `quality_command`: 613.316s
- `fresh_checkout_probes_resume`: 6.683s
- `push_create_verify_release`: 532.807s
- `distinct_channel_verification`: 0.512s
- `published_notes_audit`: 0.471s
- `post_publish_install_refresh`: 8.152s
- `post_publish_installed_readback`: 1.124s
- `release_observer`: 0.008s
- `issue_closeout_carrier`: 18.680s
- `issue_closeout`: 0.964s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-26-v8.14.1-release-observer.json`.
...
- Issue #875: `CLOSED` (https://github.com/corca-ai/charness/issues/875)
  - carrier: `direct_post_publish_commit_body`
  - manual fallback used: `False`
- Issue #876: `CLOSED` (https://github.com/corca-ai/charness/issues/876)
  - carrier: `direct_post_publish_commit_body`
  - manual fallback used: `False`
...
- Bump: `8.14.0` -> `8.14.1` (`publish-current`).
> patch, not minor: all three fixes (#825 error precedence, #875 lane attribution, #876 standalone retry) repair existing behavior; the new gate is a validator, and no registered public surface moved.