...
Advanced `charness` toward release `8.9.5` (tag `v8.9.5`) through the repo-owned release helper.

## Current Version

- previous version: `8.9.4`
- target version: `8.9.5`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 464.7s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/2556285-1789986665224163302/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.9.5-prepush-quality.json` (sha256: `73939d2c6354d34f1d281950bdfefbd104c57f53f41bf6b2d9b4fcc08c9fdea6`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.9.5`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.9.5`
...
- Response content checked for: `v8.9.5`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.9.5`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v895-release-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-21-v8.9.5-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: subagent session completed the claims review in its own context and authored the narrative file; the release driver only scaffolded the record around it
- Review narrative: `charness-artifacts/release-review/2026-09-21-v8.9.5-prepared-claims-review.md`.
- Verdict scope: 186 blocking path(s) gated this tag; 1 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.9.5`.
...
- `cli_skill_surface_gate`: 2.421s
- `quality_command`: 464.689s
- `fresh_checkout_probes_resume`: 6.453s
- `push_create_verify_release`: 424.554s
- `distinct_channel_verification`: 0.692s
- `published_notes_audit`: 0.469s
- `post_publish_install_refresh`: 9.053s
- `post_publish_installed_readback`: 1.182s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-21-v8.9.5-release-observer.json`.
