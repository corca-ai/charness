...
Advanced `charness` toward release `8.6.2` (tag `v8.6.2`) through the repo-owned release helper.

## Current Version

- previous version: `8.6.1`
- target version: `8.6.2`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 355.0s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness-prepush-quality-d3odypwh/semantic-quality.json`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.6.2`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.6.2`
...
- Response content checked for: `v8.6.2`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.6.2`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/2026-09-11-release-8-6-2.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-11-v8.6.2-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: Distinct host subagent /root/release_862_claims_review directly inspected prepared commit 0f679b138 and local evidence without modifying files or git state.
- Review narrative: `charness-artifacts/release-review/2026-09-11-v8.6.2-claims.md`.
- Verdict scope: 33 blocking path(s) gated this tag; 5 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.6.2`.
...
- `cli_skill_surface_gate`: 2.307s
- `quality_command`: 354.965s
- `fresh_checkout_probes_resume`: 4.837s
- `push_create_verify_release`: 4.839s
- `distinct_channel_verification`: 0.506s
- `published_notes_audit`: 0.401s
- `post_publish_install_refresh`: 8.714s
- `post_publish_installed_readback`: 1.045s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-11-v8.6.2-release-observer.json`.
...
> Patch: this release repairs existing Achieve planning guidance and quality adapter/catalog behavior while preserving public invocation and structured-output compatibility.