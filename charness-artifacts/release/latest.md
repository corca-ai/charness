...
Advanced `charness` toward release `8.9.4` (tag `v8.9.4`) through the repo-owned release helper.

## Current Version

- previous version: `8.9.3`
- target version: `8.9.4`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 317.9s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/1737336-1789964739117515452/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.9.4-prepush-quality.json` (sha256: `2fb310423559ba3e411098eca641bad11cf022e296837e69364a1a3a8f47217b`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.9.4`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.9.4`
...
- Response content checked for: `v8.9.4`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.9.4`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v894-release-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-21-v8.9.4-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: Fresh-checkout smoke of prepared commit af8a854ae in a /tmp clone: declared probes passed, smoke commands exit 0, packaging 8.9.4; dated observer record filed in the release probe directory.
- Review narrative: `charness-artifacts/release-review/2026-09-21-v8.9.4-prepared-claims-review.md`.
- Verdict scope: 18 blocking path(s) gated this tag; 1 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 2 finding(s) recorded in advisory scope and carried into this release record without gating this tag:
  - Concurrent quality runs can race the failure-log sweep window (pre-existing tool limitation, no repair demanded).
  - No committed 8.9.4 prepush-quality receipt; 90.7s quality figure pending final resume verification.
...
- Public release check: `gh release view v8.9.4`.
...
- `cli_skill_surface_gate`: 2.378s
- `quality_command`: 317.880s
- `fresh_checkout_probes_resume`: 6.383s
- `push_create_verify_release`: 259.899s
- `distinct_channel_verification`: 0.494s
- `published_notes_audit`: 0.451s
- `post_publish_install_refresh`: 8.416s
- `post_publish_installed_readback`: 1.048s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-21-v8.9.4-release-observer.json`.
...
> patch: two tooling repairs (failed-draft readiness refusal, stale failure-log sweep) with regression tests; no new surface, no invocation break