...
- git branch: `impl/provider-selection-20260827`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh` exited 0 in 2.4s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh`).
...
- `cli_skill_surface_gate`: 1.937s
- `quality_command`: 2.418s
- `fresh_checkout_probes_initial`: 4.751s
...
> major: removes the public handoff skill and Charness-provided SessionStart/startup routing, so callers that invoke those surfaces or depend on that automation must migrate to achieve plus AGENTS.md/docs/index.md