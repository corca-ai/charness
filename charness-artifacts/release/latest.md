...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 704.6s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/2124081-1790368440492781906/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.14.0-prepush-quality.json` (sha256: `072d4d531d82be42bf59f125530dcf24d70ddb8d830f3bfdf2b034caada3c7e9`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.14.0`, checked at `post-claims-review, pre-push`.
- Validated tree: commit `24e0b4c3c7bfb2af96b38422fd7e0c18c8e06f98` (tree `25f5bfe3236964f8419ccadef159ab22f669bac4`); a pushed tag or branch that does not contain this tree was not what this check validated.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.14.0`
...
- Response content checked for: `v8.14.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.14.0`
- HTTP status: `200`
...
- Claims review record: `charness-artifacts/release-review/2026-09-26-v8.14.0-prepared-claims-review-r2.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: subagent 01a0da2b-7e76-7242-b43d-585f1859e78f, narrative charness-artifacts/release-review/v8140-claims.md
- Review narrative: `charness-artifacts/release-review/v8140-claims.md`.
- Verdict scope: 97 blocking path(s) gated this tag; 0 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 1 finding(s) recorded in advisory scope and carried into this release record without gating this tag:
  - claims evidence re-bound with narrative addendum after validator-floor critique-conformance fix (82896c03e); original pass verdict at 621844560 by the same reviewer, scope unchanged
...
- Public release check: `gh release view v8.14.0`.
...
- `cli_skill_surface_gate`: 2.298s
- `quality_command`: 704.648s
- `fresh_checkout_probes_resume`: 13.224s
- `push_create_verify_release`: 645.376s
- `distinct_channel_verification`: 0.807s
- `published_notes_audit`: 0.537s
- `post_publish_install_refresh`: 13.809s
- `post_publish_installed_readback`: 1.925s
- `release_observer`: 0.002s
- `issue_closeout`: 0.001s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-25-v8.14.0-release-observer.json`.
...
- Bump: `8.13.0` -> `8.14.0` (`publish-current`).
