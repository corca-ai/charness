...
Advanced `charness` toward release `8.4.3` (tag `v8.4.3`) through the repo-owned release helper.

## Current Version

- previous version: `8.4.2`
- target version: `8.4.3`
- git branch: `main`
- git remote: `origin`

## Verification

- `./scripts/run-quality.sh --release --read-only` exited 0 in 189.8s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.4.3`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.4.3`
...
- Response content checked for: `v8.4.3`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.4.3`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/2026-09-05-release-8-4-3.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-05-v8.4.3-prepared-claims-review.json`.
...
- Review narrative: `charness-artifacts/release-review/2026-09-05-v8.4.3-claims-review.md`.
- Verdict scope: 73 blocking path(s) gated this tag; 6 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 6 defect(s) recorded in the advisory scope and SHIPPED KNOWN-INACCURATE rather than repaired before this tag:
  - Quality 189.0s / exit 0 is helper-measured; a gitignored local receipt corroborates pass but is not in the prepared commit.
  ...
  - Adapter-unchanged was not git-diffed against v8.4.2.
  - The product-delta half of the bump rationale is operator prose citing the critique, not a helper measurement.
...
- Public release check: `gh release view v8.4.3`.
...
- `cli_skill_surface_gate`: 2.379s
- `quality_command`: 189.848s
- `fresh_checkout_probes_resume`: 4.421s
- `push_create_verify_release`: 3.776s
- `distinct_channel_verification`: 0.643s
- `published_notes_audit`: 0.407s
- `post_publish_install_refresh`: 9.223s
- `post_publish_installed_readback`: 1.123s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-05-v8.4.3-release-observer.json`.
...
> patch, not minor: form-identity validation and contract repairs, plus empty-executable install/update refusal. No new public skill or operator command. check_schema_enum_axis is consumer_facing: false. Cheap-owner hook and mutation-recovery stubs since v8.4.2 are also patch.