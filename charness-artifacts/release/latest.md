...
Advanced `charness` toward release `8.6.0` (tag `v8.6.0`) through the repo-owned release helper.
...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 326.4s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness-prepush-quality-jtbjyucm/semantic-quality.json`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.6.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.6.0`
...
- Response content checked for: `v8.6.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.6.0`
- HTTP status: `200`
...
- No mutable source-tree pointers found (3223 body bytes).
...
- Review proof: `charness-artifacts/critique/2026-09-06-release-8-6-0.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-06-v8.6.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: Native host spawned /root/release860_claims (Sartre) with fork_turns=none, requested high-leverage adapter fields gpt-5.6-terra/medium; requested_fields_sent, applied model metadata unverified. Parent received full findings and read the new narrative; parent-delegated, findings-received. Shared-tree fingerprint release-8-6-0-claims verified clean with no drift before narrative integration.
- Review narrative: `charness-artifacts/release-review/2026-09-06-v8.6.0-prepared-claims-review.md`.
- Verdict scope: 113 blocking path(s) gated this tag; 7 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.6.0`.
...
- `cli_skill_surface_gate`: 2.169s
- `quality_command`: 326.361s
- `fresh_checkout_probes_resume`: 4.937s
- `push_create_verify_release`: 4.488s
- `distinct_channel_verification`: 0.728s
- `published_notes_audit`: 0.453s
- `post_publish_install_refresh`: 8.194s
- `post_publish_installed_readback`: 1.163s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-06-v8.6.0-release-observer.json`.
...
> minor for the additive maintained review-resolution command; supported Git callers and final closeout contracts remain unchanged, with the explicitly removed non-Git fallback disclosed.