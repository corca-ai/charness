...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 217.8s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh --release --read-only --release-prepare`); quality unestablished: pytest-release pending final resume.
- pre-push quality receipt: NOT recorded by this helper invocation, so the quality sentence above cites no durable receipt.
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.14.1`, checked at `post-bump, pre-commit`.
- Validated tree: commit `fee0eef88f351fab8aedd871ed0039c208229bc9` (tree `ab7c1c413a6a3b8d1816e94ac42e18982c7d30f0`); a pushed tag or branch that does not contain this tree was not what this check validated.
...
- `cli_skill_surface_gate`: 2.557s
- `quality_command`: 217.755s
- `fresh_checkout_probes_initial`: 6.659s
