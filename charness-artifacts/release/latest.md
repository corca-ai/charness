...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 329.6s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/3740486-1789999090625861237/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.9.6-prepush-quality.json` (sha256: `326f5bedf5a4606e5aa13084303bcd29476a7b61257f5ce36447a8660084ec07`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.9.6`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.9.6`
...
- Response content checked for: `v8.9.6`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.9.6`
- HTTP status: `200`
...
- Claims review record: `charness-artifacts/release-review/2026-09-21-v8.9.6-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: file-backed worker v896-claims (codex exec, read-only) delivered verdict block with 2 record-only findings and no code findings; report charness-artifacts/critique/workers/v896-claims/worker-report.yaml
- Review narrative: `charness-artifacts/release-review/v896-claims-r2.md`.
- Verdict scope: 229 blocking path(s) gated this tag; 1 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 2 finding(s) recorded in advisory scope and carried into this release record without gating this tag:
  - F1: closeout line conflates pending/not-requested; exact state is requested-pending-resume
  - F2: critique artifact outside worker semantic inputs; verified present, bound, and verdict-recorded
...
- Public release check: `gh release view v8.9.6`.
...
- `cli_skill_surface_gate`: 2.326s
- `quality_command`: 329.577s
- `fresh_checkout_probes_resume`: 6.486s
- `push_create_verify_release`: 344.118s
- `distinct_channel_verification`: 0.496s
- `published_notes_audit`: 0.424s
- `post_publish_install_refresh`: 7.766s
- `post_publish_installed_readback`: 1.049s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-21-v8.9.6-release-observer.json`.
