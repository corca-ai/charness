...
Advanced `charness` toward release `8.11.0` (tag `v8.11.0`) through the repo-owned release helper.

## Current Version

- previous version: `8.10.0`
...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 536.4s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/1680115-1790136871163627812/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.11.0-prepush-quality.json` (sha256: `3767680b8593fc12ae533a96b6917ea6b4e47868fb2fe223943ec417867d8551`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.11.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.11.0`
...
- Response content checked for: `v8.11.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.11.0`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v8110-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-23-v8.11.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: ephemeral native subagent session 01a0cc77, fresh context, shared checkout, read-only objective, no repo mutation; audit persisted verbatim in charness-artifacts/release-review/v8.11.0-claims-review.md
- Review narrative: `charness-artifacts/release-review/v8.11.0-claims-review.md`.
- Verdict scope: 28 blocking path(s) gated this tag; 2 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 3 finding(s) recorded in advisory scope and carried into this release record without gating this tag:
  - latest.md:18 frames quality as unestablished; resume must run the full release lane before publication.
  - latest.md:77 issue-closeout-pending line reads stale next to closeout commit 53a7a91be.
  - Tag v8.11.0 absent and push/release pending review is expected pre-publish state, not a finding.
...
- Public release check: `gh release view v8.11.0`.
...
- `cli_skill_surface_gate`: 2.334s
- `quality_command`: 536.386s
- `fresh_checkout_probes_resume`: 6.616s
- `push_create_verify_release`: 525.967s
- `distinct_channel_verification`: 0.509s
- `published_notes_audit`: 0.439s
- `post_publish_install_refresh`: 9.072s
- `post_publish_installed_readback`: 1.169s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-23-v8.11.0-release-observer.json`.
