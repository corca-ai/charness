...
Advanced `charness` toward release `8.5.0` (tag `v8.5.0`) through the repo-owned release helper.

## Current Version

- previous version: `8.4.4`
...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 358.7s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness-prepush-quality-yiwulm6o/semantic-quality.json`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.5.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.5.0`
...
- Response content checked for: `v8.5.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.5.0`
- HTTP status: `200`
...
- No mutable source-tree pointers found (3350 body bytes).
...
- Review proof: `charness-artifacts/critique/2026-09-06-goal805-release-8-5-0.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-06-v8.5.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: Canonical read-only codex_exec worker goal805-prepared-claims returned findings-received with matching typed receipt, delivery ledger and bound packet; parent consumed the pass on 2026-09-06.
- Review narrative: `charness-artifacts/release-review/2026-09-06-v8.5.0-prepared-claims-review.md`.
- Verdict scope: 352 blocking path(s) gated this tag; 6 advisory path(s) (session narrative) were reviewed but did not.
...
- Public release check: `gh release view v8.5.0`.
...
- `cli_skill_surface_gate`: 2.424s
- `quality_command`: 358.653s
- `fresh_checkout_probes_resume`: 4.913s
- `push_create_verify_release`: 5.529s
- `distinct_channel_verification`: 0.510s
- `published_notes_audit`: 0.518s
- `post_publish_install_refresh`: 14.101s
- `post_publish_installed_readback`: 1.193s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-06-v8.5.0-release-observer.json`.
...
> Minor: add exact multi-target review and per-issue classification support, and release preparation/final-resume scheduling without incompatible invocation changes. The delegation pilot establishes no efficiency gain.