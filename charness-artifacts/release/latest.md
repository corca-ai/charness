...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 163.0s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh --release --read-only --release-prepare`); quality unestablished: pytest-release pending final resume.
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.7.1`, checked at `post-bump, pre-commit`.
...
- `cli_skill_surface_gate`: 2.141s
- `quality_command`: 163.021s
- `fresh_checkout_probes_initial`: 5.459s
...
> patch, not minor: two bug repairs (muse lane workspace root/trust; mutation probe budget sharing and match ordering; StrykerJS sandbox crash on the dangling-symlink fixture) plus test and doc support. No public surface added, moved, or removed.