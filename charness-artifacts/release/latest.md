...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 383.7s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh --release --read-only --release-prepare`); quality unestablished: pytest-release pending final resume.
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.11.0`, checked at `post-bump, pre-commit`.
...
- `cli_skill_surface_gate`: 2.280s
- `quality_command`: 383.664s
- `fresh_checkout_probes_initial`: 6.708s
...
> minor: alongside the divergence-refusal repair, adds operator-facing reclamation behavior (prune reclaim with age reports, empty-lane release, patch-equivalence) that existing users adopt without migration.