...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 167.8s at `post-bump, pre-commit`, measured by this helper (`./scripts/run-quality.sh --release --read-only --release-prepare`); quality unestablished: pytest-release pending final resume.
...
- `cli_skill_surface_gate`: 2.314s
- `quality_command`: 167.785s
- `fresh_checkout_probes_initial`: 6.604s
