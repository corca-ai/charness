...
Advanced `charness` toward release `8.12.0` (tag `v8.12.0`) through the repo-owned release helper.

## Current Version

- previous version: `8.11.1`
...
- `./scripts/run-quality.sh --release --read-only` exited 0 in 585.6s at `post-claims-review, pre-push`, measured by this helper (`./scripts/run-quality.sh --release --read-only --receipt-json=/home/hwidong/.cache/tmp/charness/runtime/811b9f8f8a808bfa/scratch/release-prepush-quality/3906548-1790287219903517312/semantic-quality.json`).
- pre-push quality receipt: `charness-artifacts/release/8.12.0-prepush-quality.json` (sha256: `7bdb3ff26d49cbf7fde284b6b8e9f9afabddf8a785b1ac6e69e9c6ba27a242fb`).
- `current_release.py` reported no version drift across 4 versioned surface(s), with 1 presence-only surface(s) not version-checked against target `8.12.0`, checked at `post-claims-review, pre-push`.
...
- GitHub release record: verified URL `https://github.com/corca-ai/charness/releases/tag/v8.12.0`
...
- Response content checked for: `v8.12.0`
...
- Channel URL: `https://github.com/corca-ai/charness/releases/tag/v8.12.0`
- HTTP status: `200`
...
- Review proof: `charness-artifacts/critique/v8120-critique.md`.

## Claims Review

- Claims review record: `charness-artifacts/release-review/2026-09-25-v8.12.0-prepared-claims-review.json`.
- Claims review verdict: `pass`.
- Observer distinctness: `separate-agent-context`.
- Recorded signal: subagent claims-review-v8120 log 01a0d56b-6fcc-7751-ac5b-fdb870761bee
- Review narrative: `charness-artifacts/release-review/v8120-claims-review.md`.
- Verdict scope: 243 blocking path(s) gated this tag; 4 advisory path(s) (session narrative) were reviewed but did not.
- Advisory findings: 3 finding(s) recorded in advisory scope and carried into this release record without gating this tag:
  - Quality-gate pass line lacks a durable receipt in the prepared record.
  - Drift-check claim cites no output or receipt.
  - Bump rationale not recorded in release notes.
...
- Public release check: `gh release view v8.12.0`.
...
- `cli_skill_surface_gate`: 2.316s
- `quality_command`: 585.553s
- `fresh_checkout_probes_resume`: 6.823s
- `push_create_verify_release`: 506.868s
- `distinct_channel_verification`: 0.588s
- `published_notes_audit`: 0.484s
- `post_publish_install_refresh`: 8.709s
- `post_publish_installed_readback`: 1.135s
- `release_observer`: 0.001s
- `issue_closeout`: 0.000s

## Release Observer Record

- Durable observer record: `charness-artifacts/probe/2026-09-24-v8.12.0-release-observer.json`.
