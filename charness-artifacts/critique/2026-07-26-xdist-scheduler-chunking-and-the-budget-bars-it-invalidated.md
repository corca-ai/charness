...
- F3 | bin: act-before-ship | evidence: strong | ref: .agents/quality-adapter.yaml:210 | action: fix | note: first-pass bars were sized from the post-change slice while enforcement reads the full-window median, leaving `run-quality-read-only` at 1.02x and `run-quality-full` at 1.03x of the blocking basis. The windows were run until the ENFORCEMENT BASIS was post-change and each margin verified — full 20/20 convergence for `pytest` only; the two aggregates reached 13/20, which is why F12 below exists.
...
- F10 | bin: act-before-ship | evidence: strong | ref: scripts/run_standing_pytest.py:290 | action: fix | note: SUPERSEDES this finding's first draft, which binned the suppression coupling `over-worry` on the reasoning that `off` is a deliberate debugging choice. That was wrong twice. It named the wrong label — `pytest` at 58500 survives the ~52s pre-flag regime, while `run-quality-full` at 62000 sits BELOW this file's own recorded 67172 pre-flag basis — and it ignored the two INVOLUNTARY suppression paths (xdist < 3.2, an unrelated `PYTEST_ADDOPTS` tuning), which no operator chose and which produce a blocking red with nothing regressed. `choose_sched_chunk` now returns a suppression reason and `build_pytest_command` prints it to stderr.
- F12 | bin: act-before-ship | evidence: strong | ref: .agents/quality-adapter.yaml:262 | action: fix | note: `run-quality-read-only` and `run-quality-full` were left enforcing on windows only 13/20 post-change, so a pre-flag sample still sat in each basis while F3's comment implied convergence. Both windows were then run to 20/20 and every bar re-verified against the FULL window rather than a slice: `pytest` 1.54x worst / 1.71x median, `run-quality-read-only` 1.41x / 1.56x, `run-quality-full` 1.40x / 1.62x.
...
- Producer: `scripts/run_standing_pytest.py` — owner of the standing pytest argv it builds, including worker width and now scheduling granularity. Not the SOLE owner: pytest prepends both `PYTEST_ADDOPTS` and ini `addopts`, so those co-own the final argv. The runner defers to the former explicitly and silently overrides the latter.
