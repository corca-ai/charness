...
Critique and counterweight guidance currently says fresh-eye review requires
...
Clarify the public `critique` contract and shared subagent capability check:
...
- **No new public skill.** This is a clarification to the existing critique
  execution contract.
- **Central rule lives in `subagent-capability-check.md`.** `spec`, `quality`,
  and `handoff` already reference that shared check.
- **`critique/SKILL.md` gets a concise hook.** Detailed nested-runtime behavior
...
- `rg -n "parent-delegated|nested-delegated|recursive" skills/public/critique`
  finds the delegation context rule.
- `rg -n "already the bounded fresh-eye subagent|do not spawn another" skills/public/critique`
