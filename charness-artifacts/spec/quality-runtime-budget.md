...
update --repo-root . --no-pull`로 SKILL 변경분을 현재 working tree에
...
- pytest budget = **25000ms** hard fail. 점진(30→25→20) 안 함. 슬라이스 1이
  pytest 자체를 18-19s standalone로 줄였고, run-quality.sh의 queue/log 래퍼와
  머신 부하 변동을 합쳐 측정값은 18-22s 범위. 처음 22s로 박았으나 머신 noise로
  flake할 가능성이 있어 25s를 default로 확정. 60s → 25s 회귀 방어 효과는
  동일하게 유지하면서 호스트별 부하 차이를 흡수.
...
이 spec 문서: `charness-artifacts/spec/quality-runtime-budget.md`. 구현 중 실측으로
드러나는 사실(예: xdist race가 있는 파일, 실제로 줄여진 ms, 어댑터 스키마
세부)이 있으면 여기를 갱신한다. closeout 시 latest.md(`Recommended Next
