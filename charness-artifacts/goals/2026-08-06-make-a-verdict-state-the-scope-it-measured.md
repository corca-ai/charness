...
- Issue closeout: #490, #488, #489, #487 CLOSED. Carrier: direct-commit `2270eb14` — the ledger lives in that COMMIT MESSAGE, not in a comment body, because that is where `validate-closeout-draft` reads it for this carrier; the fixes themselves shipped earlier in `a5b5d0e8`, `8573f862`, `3f7e0d04`, `c09c7f4a`. Proof: `validate-closeout-draft --classification bug --carrier direct-commit` → `draft_verified`, and `verify-closeout --commit-ref 2270eb14 --expect-state CLOSED` → `status: verified`, `ok: true`, state read back through the adapter. A DELEGATED resolution critique ran BEFORE the close call ([2026-08-06-issue-487-488-489-490-resolution-critique.md](../critique/2026-08-06-issue-487-488-489-490-resolution-critique.md)) and REFUSED two of the four as scoped: it required the CI re-read recorded below, an issue for the `mutation_testing` nested-sub-key coarseness (#493), and either a repair or a filing for `upsert_goal.py`'s remaining argv channel (#494). **#487 is therefore closed on `append_slice_log.py` ONLY.** #491/#492/#493/#494 are filed-not-closed residue.
