...
- Current slice: slices 1-3 done. Slice 4 is next.
- Next action: slice 4 — reproduce the specdown churn first (full quality gate,
  then `git status --short .charness/specdown/`); if clean, close it as already
  fixed by the 2026-07-22 change and leave the slot empty per operator decision.
