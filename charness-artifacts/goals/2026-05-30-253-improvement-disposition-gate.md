...
### Slice 3: Rung-2 mandate + three doc surfaces + sync

- Objective: Define the fresh-eye disposition-review mandate (rung 2: read retro Next Improvements + goal Auto-Retro, record a per-improvement verdict, bind via Disposition review: line), and reconcile all three doc surfaces honestly (lifecycle.md, SKILL.md, prescribed-skill-closeout-contract.md). Sync the plugin mirror; run the doc-slice verify set.
- Why this approach: Sync-before-verify barrier; behavior must be documented on all three surfaces (repo-fit R4: SKILL.md is the third) before closeout. The per-improvement-vs-per-goal scope distinction reconciles 'exactly two/mandatory' with the opt-out without contradiction.
- Commits: (this doc commit)
- What changed: lifecycle.md: added the per-goal opt-out (scope vs per-improvement), a new 'Disposition gate — two rungs (#253)' subsection (rung-1 floor: block-the-blank + review-ran-evidence presence/binding-only; rung-2 fresh-eye reviewer; honest agent-backed limit; narration-nonblocking-vs-review-existence-blocking asymmetry), and the Disposition review: evidence line + grandfather rule + the 251 re-check-not-re-refusal note in the After-phase evidence gate. SKILL.md: narrow per-improvement/per-goal reconciliation + disposition-gate step + 2 guardrails (opt-out, do-not-classify). prescribed-skill-closeout-contract.md: disposition_review in the achieve evidence row + a full 'Improvement-Disposition Gate (#253)' section. allowlist: +achieve:artifact:critique (rung-2 review artifact, mirrors issue/release:artifact:critique). Re-synced plugin mirror.
- Alternatives rejected: REJECTED making lifecycle.md namespace-neutral to dodge the ownership-overlap gate: the disposition review IS a critique-class artifact, so critique/ is the correct home and the explicit allowlist entry (matching issue:artifact:critique + release:artifact:critique) is the established pattern, not a workaround. REJECTED a broad SKILL.md rewrite: kept it a narrow widening (157->174 lines, under the 200 ceiling).
- Targeted verification: Doc-slice verify set all green: check-markdown 0 errors (fixed a wrapped inline code span), check_skill_ownership_overlap ok (allowlist=24), validate_skills exit 0, check_doc_links exit 0, check_command_docs ok, validate_cautilus_proof (deterministic owns 2 prompt-affecting paths; no live run), py_compile ok, public-skill validation + dogfood ok. validate_packaging + sync clean.
- Test duplication pressure: No tests added this slice (docs only). No duplicate-pressure change.
- Critique: Self-review: confirmed the three surfaces agree on the two-rung model, the grandfather rule, and the honest agent-backed-non-determinism limit; SKILL.md stays a narrow widening (no contradiction with 'exactly two'). Cautilus posture: prompt-affecting but on-demand — no live Cautilus run triggered by prose diffs (deterministic validate_cautilus_proof owns the 2 prompt-affecting paths).
- Off-goal findings: check-markdown caught a wrapped inline code span (skipped: host-blocked-subagent: <detail>) — fixed by shortening the code span to host-blocked-subagent. check_skill_ownership_overlap required the achieve:artifact:critique allowlist entry — added with reason.
