...
- Current slice: slices 1-2 DONE. Slice 3 next: `check_mutation_run_proof`'s empty-range `provable`.
- Next action: slice 3 — decide WHICH input carries the range fact before touching `check_mutation_run_proof`; the classifier has no head SHA and no git access by design, so adding them is a design change, not a fix.
