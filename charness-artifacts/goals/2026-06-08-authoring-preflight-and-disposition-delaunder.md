...
- workflow (post-closeout waste retro, added after a `dispose 제대로 됐나요?` review): run the broad `surface_obligations` suite before committing a slice that rewires `run_slice_closeout`/the structural sweep — the Slice-3→4 reversal was caught late. Disposition: applied: persisted to recent-lessons this run (next-time checklist).
- workflow (post-closeout waste retro): extend the author-time preflight to the goal-closeout/coordination-floor surfaces — the closeout line-shapes were discovered by failing the complete-flip ~4×, the same recurrence class for the surface this goal did not cover. Disposition: none — deferred: recorded as the primary follow-on in the early-close report; a candidate future issue carrying a `recurs:` lineage marker, not a fresh narrow re-file.
