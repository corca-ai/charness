...
- Critique: Scenario review folded: announcement routing, draft artifact path, and draft-first behavior are unchanged. Fresh-eye final critique found one blocker: `thread_reply` before `parent` was still marked executable. Fixed before closeout by requiring each `thread_reply` to have a preceding parent output and adding resolver/capability regression tests; live Cautilus was not required by planner.
