...
### Slice 4: Issues 252 and 241 setup portability extensions

- Objective: Close the setup/create-skill portability pair: compact AGENTS contracts for setup normalization and host-owned extension metadata for the create-skill adapter.
- Why this approach: Both issues protect host portability. #252 keeps root AGENTS as a compact host-read-time bootstrap surface without weakening subagent or routing safety; #241 keeps downstream create-skill customization in adapter YAML instead of forks of Charness-authored resolver code.
- Commits: ec229ea Support compact setup and adapter extensions
- What changed: `setup` inspection now accepts compact Subagent Delegation when it carries a standing delegation request, canonical scopes, host-block reporting, and affirmative no-same-agent-substitute wording; compact Skill Routing is accepted only when a line directly calls/invokes/runs `find-skills` at startup for discovery/routing before broader exploration. `create-skill` adapter resolution now preserves a `host_extensions` mapping and top-level `x-*` blocks, rejects non-mapping `host_extensions`, and documents the extension namespace. Public-skill dogfood notes and plugin mirrors were updated.
- Alternatives rejected: Did not accept arbitrary custom Skill Routing blocks that merely mention `find-skills`; did not accept compact delegation text that permits same-agent substitutes; did not add a host-specific schema registry for `create-skill` because a passthrough namespace is enough for the current host-extension need.
- Targeted verification: PASS: pytest -q tests/quality_gates/test_setup_inspect_policy.py tests/quality_gates/test_create_skill_adapter.py (42 passed); PASS: ruff check touched script/tests; PASS: python3 -m py_compile touched scripts; PASS: python3 scripts/validate_public_skill_dogfood.py --repo-root .; PASS: python3 scripts/validate_skills.py --repo-root .; PASS: python3 scripts/validate_packaging_committed.py --repo-root .; PASS: python3 scripts/run_evals.py --repo-root . --scenario-id setup-compact-skill-routing-discoverability; PASS: python3 scripts/run_evals.py --repo-root . --scenario-id representative-skill-contracts; PASS: python3 scripts/run_slice_closeout.py --repo-root . --ack-cautilus-skill-review (completed, including broad pytest and agent-browser guard).
- Test duplication pressure: Added focused regression coverage to existing setup and create-skill adapter test files rather than adding a new suite. Length gate warns that `scripts/setup_agent_docs_lib.py` is 474/480 and `tests/quality_gates/test_setup_inspect_policy.py` is 778/800; future work should split before appending more.
- Critique: Fresh-eye reviewer Volta found two #252 blockers: compact delegation accepted permissive same-agent language, and compact routing accepted keyword co-occurrence that could hide custom routing drift. Both were fixed with stricter matching and regression tests; final follow-up reported No remaining blockers. No #241 blockers were found; the only residual risk was additive JSON output churn from the new `host_extensions` default.
- Off-goal findings: Cautilus planner reported proof_kinds none / next_action none under ask-before-run policy. Maintained scenario registry was inspected; no registry mutation was made because deterministic resolver/inspection tests plus existing setup compact-routing and representative skill-contract scenarios cover this slice's changed behavior.
...
- Current branch proof after Slice 4 implementation: `git status --short
