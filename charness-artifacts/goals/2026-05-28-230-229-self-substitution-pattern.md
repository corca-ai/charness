...
### Slice 5: issue closeout guard (#230 Waste 1b)

- Objective: Extend the shared closeout helper to issue resolution closeout - refuse closeout without a Critique carrier-header line; integrate cleanly with the existing issue_tool.py verify-closeout
- Why this approach: Second of three sibling surfaces per the goal scope; the shared helper from slice 3 makes this slice cheap. The existing verify-closeout ledger checks remain unchanged - the new check is additive and runs first
- Commits:
- What changed: 1) skills/public/issue/scripts/issue_verify_closeout.py - new CRITIQUE_REQUIRED_CLASSIFICATIONS tuple, _CRITIQUE_LINE / _CRITIQUE_BLOCKED regexes, _extract_critique_value() (strips code fences first), _check_resolution_critique() (extracts the Critique: line, maps path -> evidence and blocked-signal -> host-blocked-subagent skip via the shared helper, no-ops for question/decision-needed). verify_closeout() now folds the new check into ok/status and returns the structured resolution_critique_check field. 2) tests/quality_gates/test_issue_closeout_verifier.py - _bug_closeout_body grew an optional critique_line argument defaulting to a valid blocked-skip line; existing 16 acceptance tests still pass under the new contract. 5 new tests cover missing Critique line, real artifact path, too-terse blocked signal (rejected by the helper's 40-char floor), question classification skipping the gate, and feature classification with blocked-skip. 3) skills/public/issue/references/closeout-discipline.md - new ## Resolution-Critique Carrier Header subsection documenting the contract for bug/feature/deferred-work and citing the spec doc.
- Alternatives rejected: Extending issue_verify_closeout's _classification_requirements field set to add resolution_critique (rejected - this was the slice-1 F1 critique blocker; the shared-helper carrier-header pattern is the F1 fold). Making the gate opt-in via a --require-resolution-critique flag (rejected - user acceptance criterion explicitly says it must be gated by default, not optional). Padding too-terse blocked signals in the wrapper to force a meaningful error (rejected mid-slice when it accidentally tripped the 40-char floor; cleaner to pass the raw signal through and let the helper's MIN_SKIP_LENGTH enforce honesty).
- Targeted verification: validate_skills exit 0; check_doc_links exit 0; ruff clean on changed Python. Full test suite for the affected modules: 55 passed across test_issue_closeout_verifier.py (21 tests), test_goal_artifact_lib.py (23 tests), test_prescribed_skill_executed.py (11 tests). The 5 new tests prove: missing critique fails (resolution_critique_check.missing == [resolution_critique]); real artifact path accepted; too-terse blocked signal rejected by 40-char floor; question classification skips gate (skipped_classification field); feature classification with proper blocked-skip accepted.
- Test duplication pressure: +5 tests in test_issue_closeout_verifier.py reusing the existing _seed_commit helper. _bug_closeout_body grew an optional argument - clean expansion of an existing fixture, no duplication. Adjacency check deferred to bundle boundary.
- Critique: Slice-level scoped (no new design surface emerged; F1 from slice 1 already designed the cleanest non-duplicating integration; this slice executed that design).
- Off-goal findings: None this slice
- Lessons carried forward: When extending a tightly-tested existing module, update the shared test fixture to reflect the new contract rather than adding a parallel fixture - keeps the test surface uniform and forces the contract to apply to every existing assertion. The 40-char skip-reason floor is the honest enforcement point; do not pad terse signals in the wrapper because that defeats the floor's purpose.
- Metrics: when available - host-log probe deferred to slice 8

### Slice 6: release closeout guard (#230 Waste 1c)

- Objective: Extend the shared closeout helper to release standalone-critique closeout - refuse publish unless --critique-artifact or --critique-blocked is satisfied through the shared helper
- Why this approach: Third of three sibling surfaces per the goal scope. The shared helper from slice 3 made this trivially additive at one CLI surface point
- Commits:
- What changed: 1) skills/public/release/scripts/publish_release_preflight.py - new enforce_release_critique_gate() that loads the shared helper, requires exactly one of {critique_artifact, critique_blocked}, maps the blocked signal through host-blocked-subagent prefix, and raises SystemExit with the helper's structured report when refused. 2) skills/public/release/scripts/publish_release_cli.py - wires the gate immediately after validate_critique_artifact_arg, adds the new --critique-blocked argv flag. 3) skills/public/release/SKILL.md - adds the new reference listing (199 to 200 lines, exactly at the cap). 4) skills/public/release/references/closeout-critique-gate.md - new reference documenting the runtime contract and operator flow. 5) tests/quality_gates/test_release_publish.py - _run_publish_patch helper centrally injects --critique-blocked when the caller did not specify a flag, so existing tests continue to reach their downstream assertions without per-test edits. 6) tests/quality_gates/test_release_publish_critique_artifact.py - 3 new tests covering the gate: no critique flag, blocked-signal too terse, both flags at once. Original untracked-artifact test preserved.
- Alternatives rejected: Making --critique-blocked just lengthen the prose without the host-blocked-subagent enum (rejected - that would let any free-text reason in, defeating slice 1 F2 fold). Per-test edits to inject --critique-blocked everywhere (rejected - one shared helper update is cheaper and contracts every existing test under the new policy). Adding the gate at the artifact-write step instead of at preflight (rejected - operators see the failure cleaner before any mutation happens).
- Targeted verification: validate_skills exit 0; check_doc_links exit 0; ruff clean on changed Python (F841 unused-variable caught and fixed); markdown-lint exit 0; pytest tests/quality_gates/test_release_publish.py + tests/quality_gates/test_release_publish_critique_artifact.py = 16 passed (3 new). Shared _run_publish_patch helper now also covers the new gate by injecting --critique-blocked centrally, so the 8 existing failures from the mid-slice are fully unblocked.
- Test duplication pressure: +3 tests in test_release_publish_critique_artifact.py using existing _seed_publish_release_repo. _run_publish_patch in test_release_publish.py grew the central helper without splitting fixture surface. Adjacency check deferred to bundle boundary.
- Critique: Slice-level scoped (cadence designated slice-level critique for slices 1/6/7; slice 5 is a sibling extension of slice 4's pattern with no new design surface).
- Off-goal findings: None this slice
- Lessons carried forward: When a CLI gate fires mid-flow, central test-helper injection beats per-test edits - especially when the gate is meant to apply universally. The slice-3 shared helper paid off here: slice 5 took ~25 lines of new preflight code plus a CLI flag, no helper rework. The 200-line SKILL.md cap allows new reference listings; the file went 199 to 200 cleanly.
- Metrics: when available - host-log probe deferred to slice 8

### Slice 7: Push-gate batching (#230 Waste 3)

- Objective: Smarter broad-gate routing so docs/artifact-only repushes skip redundant repetition without weakening correctness. Stop condition: any plugins/ .claude-plugin/ .agents/plugins/ touch unconditionally forces the full gate
- Why this approach: User chose full-fix scope. Correctness risk demands explicit design + slice-level critique
- Commits:
- What changed: 1) scripts/classify_push_diff_lib.py (~200 lines) - pure-function classifier with UNCONDITIONAL_FULL_GATE_PREFIXES (the stop-condition prefixes), DOCS_ARTIFACT_PREFIXES allowlist, FULL_GATE_PATTERN_HINTS (source/test/config + top-level dotfile catch-all per slice-7 F3), classify() returning structured report, resolve_diff_range() + changed_paths_from_git() helpers. 2) scripts/classify_push_diff.py (~85 lines) - CLI wrapper supporting --paths-stdin (used by the hook) and --diff-range. 3) .githooks/pre-push - reads pre-push stdin to derive correct <remote-sha>..<local-sha> per pushed ref (slice-7 F2), classifies the union of diffs via --paths-stdin, branches: full gate vs docs-only subset (15 phases via CHARNESS_QUALITY_LABELS env per slice-7 F1) vs forced-full via CHARNESS_FORCE_FULL_GATE=1. New refs, ref deletions, and unresolvable upstream all force the full gate. 4) tests/quality_gates/test_classify_push_diff.py - 15 tests covering docs-only, empty diff, the three unconditional prefixes individually, source/test/githooks/pyproject paths, non-allowlist top-level dirs, dedupe, charness-artifacts subdirs, dotfile catch-all (slice-7 F3 covering 7 dotfile/dir examples), and CLI smoke. 5) docs/conventions/operating-contract.md - Local Enforcement Policy section grew the decision matrix documenting both routes, the operator override env var, and the new-ref/delete/unresolvable-upstream forced-full path.
- Alternatives rejected: Skip-on-docs-only entirely (rejected by slice-7 critique F1: would silently land doc-link, slug-drift, current-pointer regressions that the docs-only subset still catches; the full skip lost honesty). Fixed origin/<branch> diff range (rejected by slice-7 critique F2: misses per-ref correctness for force-push, rename, non-origin remote; new hook reads pre-push stdin). Implicit (no env var override) bypass for dotfile additions (rejected by slice-7 critique F3: future maintainer foot-gun; explicit dotfile catch-all in FULL_GATE_PATTERN_HINTS instead). Pushing the new gate live during this slice to verify it (deferred to user verification at goal closeout; preexisting find-skills inventory drift on integrations IDs vs file basenames blocks any push attempt and is unrelated to slice 7).
- Targeted verification: Unit tests: 15 classifier tests + 14 prior tests = 49 total green in slice 7 suite. Hook syntax: bash -n exit 0; shellcheck clean after underscore-prefix for unused captured fields. Live exercise: ran the docs-only subset via CHARNESS_QUALITY_LABELS env exactly as the hook would; 14 phases passed + 1 phase (validate-current-pointer-freshness) caught real preexisting drift (which is itself the F1 vindication - the subset would have caught the drift the bare-skip approach would have missed). The drift is recorded as Off-Goal Finding below.
- Test duplication pressure: +15 tests in test_classify_push_diff.py; pure-function classifier unit tests, no I/O setup. CLI smoke via subprocess for the --paths-stdin contract that the hook uses. Adjacency check deferred to bundle boundary.
- Critique: Bounded fresh-eye reviewer (standard tier, parent-delegated; agentId af42d944e883b06d8). Four angles probed: correctness-regression of docs-only skips, diff-range correctness for force-push/cross-remote, bypass via top-level dotfiles, tag-only / new-branch push semantics. Triage acted: F1 docs-only subset (Act Before Ship - folded with CHARNESS_QUALITY_LABELS in the hook running ~15 docs/artifact phases in ~13s instead of skipping); F2 stdin-derived diff range (Act Before Ship - folded by reading <remote-sha>..<local-sha> per pushed ref and treating zero SHAs as new-ref forced-full); F3 dotfile catch-all (Bundle Anyway - folded with re.compile(r'^\.[^/]+') in FULL_GATE_PATTERN_HINTS plus a 7-path test). F4 tag-only docstring (Over-Worry - addressed by updating resolve_diff_range docstring; module-level wording stays accurate).
- Off-goal findings: Preexisting find-skills inventory schema drift: charness-artifacts/find-skills/latest.json.inventory.integrations entries use renamed IDs (github-worker, google-workspace-worker) while integrations/tools/*.json file basenames are still github-gh.json and gws-cli.json. validate_current_pointer_freshness expects file-basename matching and fails. Not introduced by slice 7; caught by the slice-7 docs-only subset (which the F1 fold added). To file: github-gh.json -> github-worker.json + gws-cli.json -> google-workspace-worker.json renames OR teach the validator to accept the new id-based references.
- Lessons carried forward: The slice-7 critique probably saved this slice from shipping a real regression: F1 (docs-only skip would land link/slug/pointer regressions) was a concrete miss the angle pass surfaced. F2 (stdin-derived diff range) caught a quiet correctness gap in the obvious-looking <branch>..HEAD baseline. F3 (dotfile catch-all) was a 1-line edit that removed a future foot-gun for free. Per the goal cadence, slice-level critique for non-trivial gate-routing was the right cost; the same review on slice 4/5 would have been overkill.
- Metrics: when available - host-log probe deferred to slice 8. Direct: docs-only subset runtime ~13s vs full gate ~100-120s (~10x reduction on bookkeeping pushes; full gate still runs unconditionally for the three plugin-export prefixes and any code/test/config path).
...
- **(Both findings below filed as [#233](https://github.com/corca-ai/charness/issues/233).)**
...
- **Prescribed-skill conclusions need user-message surfacing, not just file persistence (post-closeout user correction).** The slice 3 / 4 / 5 gates verify that the prescribed sub-skill produced an artifact (file exists, non-empty). They do **not** verify that the artifact's substantive findings were narrated in the user-facing conversation. This session reproduced the gap one layer up: the closeout retro artifact carried a full efficiency analysis, but the slice-8 chat summary listed raw metrics only. The user correctly flagged this as "회고와 같이 안 나옴" (the analysis didn't come out with the retro). Same shape as #230 Waste 1, one layer up — the agent ran the skill but inline-paraphrased its conclusions when relaying them. This bundles with the F1 finding above into one follow-up issue: gates today enforce necessary-but-not-sufficient conditions (file exists / artifact bound), and the next layer is enforcing that the artifact's *meaning* travels with it (binding to the goal, surfacing to the user). Smallest fix sketch (deferred): extend the achieve After-phase contract in `references/lifecycle.md` to require an explicit Efficiency Narrative line in the user-facing closeout message, plus a self-test that fails when the closeout response body is below a length floor or omits structured metrics narration. To file as part of the same follow-up issue as F1.
