...
- RESOLVED, and it was resolved BEFORE this run rather than by it: `#562`'s
  direction is option 1 — drop the owner-inspection locator pin, keep the source
  snapshot. `#562`'s own body records that as the operator's stated preference at
  filing time. This queue briefly carried it as an open question; that was this
  run re-asking a decision already taken, and the successor claims `#562` with the
  direction rather than with the question. What is NOT decided and does not block:
  nothing — the blast radius (~35 `sha256` references, a schema bump, the mirror)
  is a budget fact, not a decision.
