...
- Current slice: 4 — the two lessons a gate cannot hold, written where the next session reads them.
...
- Slice 3 premise check (RESOLVED — verdict: SHAPE, not parser):
...
- Slice 3 review: TWO bounded rounds — the round-2 gap recorded earlier is now
  ...
  literally instructs authors to use — was invisible.
- **BLOCKING off-goal regression, see `## Off-Goal Findings`:** slice 2's
  `run-quality.sh` edit invalidated the checked-in `#514/#515/#518` source-freeze
  receipt. THREE tests are red. Proven by bisection. Deliberately NOT re-stamped:
  that would assert an inspection I did not perform, and those issues belong to a
  different goal.
- Next action: decide the freeze regression with the operator (it is another
  goal's evidence), then slice 4.
...
| 3 | Closeout ledger states population and removals separately | The same arithmetic error blocked three of four closeouts at the resolution-critique stage | Blocked phrasing refused, repaired phrasing passes; SHAPE not parser (premise refuted the plan's wording). Round 2 NOT run — recorded as a gap | done (round-2 gap) |
...
### Slice 3: A consolidation ledger states population and removals as two numbers

- Objective: Make the arithmetic that blocked three of four consecutive closeouts unwritable, rather than detectable. `four implementations, three consolidated` becomes `population: 4; removed: 2`.
- Why this approach: Same failure, same way, three times: the owner counted among the things consolidated. It blocked at the resolution-critique stage each time, which is late and expensive.
- Commits: one commit on `main` (local); nothing pushed
- What changed: NEW `skills/public/issue/scripts/issue_closeout_ledger_counts.py` — `evaluate()`, `missing_sibling_ledger_fields()` (which absorbs the pre-existing decision/proof rule so both sibling-field shape rules have one owner), `SIBLING_RULE_DESCRIPTIONS`, and `rule_reason()`. `issue_verify_closeout_body.py` delegates the whole bug sibling block. `issue_verify_closeout.py` emits `missing_field_reasons`. `check_issue_closeout_commit_msg.py` renders them (new `_ledger_field_lines` helper). `describe_closeout_draft_shape.py` renders both rules FROM the owner's constant instead of hand-typing one. NEW `tests/quality_gates/test_issue_closeout_ledger_counts.py` (10 tests).
- Alternatives rejected: REJECTED a prose parser that judges the arithmetic — the goal's Non-Goals forbid a gate an operator learns to ignore, and deciding whether an English count is WRONG is the surest way to build one. The floor never checks arithmetic; `population: 4; removed: 9` passes, pinned by a test. REJECTED chasing the duplicate-hash rotation in `issue_verify_closeout_body` after taking the genuine consolidation: slice 1 MEASURED that letting a rotated hash drive a proof surface's design ships a latent false green.
- Targeted verification: PREMISE CHECK (verdict BEFORE the build): **HOLDS on frequency, REFUTED on remedy shape.** The frequency is established by the predecessor's own record ("the blocking finding in all three closeouts, always the same way"). But the slice plan says the phrasing should "fail a check rather than a reviewer", and the failing text is PROSE. Verdict: SHAPE, not parser — make the ambiguity unwritable instead of detectable, which is the same move slices 1 and 2 made and needs no English parsing. Recorded as explicitly partial in a separate commit before building, rather than guessed. — PROOF: 10 tests. `run_slice_closeout.py --skip-broad-pytest --ack-cautilus-skill-review`: `Closeout verdict: completed`. Reviewer boundary `parent-attributed`, zero undeclared. Every review claim verified by execution before acting: the authoring reference does say BUNDLED, `fold` did match `folder`, an issue reference did trigger a refusal, `matches found: 12` did become the population, and word-form labels were refused. MUTATION: 5 mutants, 5 killed — counting-claim trigger always true (the wolf-crier direction), label sets made to overlap, `and` -> `or`, the label anchor dropped, and the CALL SITE removed.
- Test duplication pressure: `check_dup_ratchet.py` hard-blocked once, on two adjacent list-building functions in `issue_verify_closeout_body`. The genuine consolidation was taken (both sibling shape rules moved to one owner); the family persisted as a length-hash rotation and was classified `intentional` with the measured reason from slice 1 cited.
- Critique: ONE bounded fresh-eye round, delegated. It endorsed the shape-not-parser trade and then found the floor barely fired on real text. THREE P1s. (1) The verb vocabulary was written from the ONE measured sentence rather than from the authoring surface that produces these sentences — so `bundled`, the verb `references/causal-review.md` literally instructs authors to use, was invisible. `four sibling sites, three bundled into this fix` sailed straight through the gate written to catch that exact shape. (2) The library built a six-line diagnosis and the only consumer read `["ok"]` and dropped it, so an author blocked at the pre-commit boundary got one unexplained snake_case token under a prefix ("missing or placeholder") that was factually wrong for a shape finding — and the same file already carried a comment naming that class as measured. (3) `describe_closeout_draft_shape.py`, whose stated contract is that it never re-declares a rule, still hand-typed one sibling rule and never learned the new one; its drift guard structurally could not see it. Also found: `fold` matching `folder` and `merg` matching `merger`; issue references and ISO dates read as counts; `found`/`total` as population labels, so a grep hit count silently became the population AND was DISPLAYED as the verdict's first number; word-form labels refused while word-form claims were accepted; the floor satisfiable by APPENDING labels while leaving the wrong arithmetic in place; and one genuinely vacuous test (it named distance and tested punctuation, leaving the span width with zero test pressure). All repaired except the append-satisfiability, which is inherent to a shape rule and is the reason surfacing the reason (P1-2) matters. NOT re-reviewed: this is round 1 and its repairs are unreviewed, which the two-round cap would normally cover — recorded as a gap rather than claimed.
- Off-goal findings: **Slice 2's `run-quality.sh` edit invalidated a checked-in source-freeze receipt.** `charness-artifacts/spec/2026-08-07-issue-514-515-518-freeze-receipt.json` pins `scripts/run-quality.sh` as an inspected locator, and adding `--require-git-file-listing` changed its hash. Three tests now fail: `test_issue_source_freeze.py::test_the_checked_in_charness_freeze_for_514_515_518_validates`, `test_closeout_bundle.py::test_this_repo_is_currently_closeout_bundle_ready`, `test_closeout_headroom_and_mirror_gate.py::test_gate_passes_on_isolated_repo_copy_in_sync`. Proven by bisection: reverting ONLY that one file to the session-start version turns all three green. The freeze is WORKING — a source file changed after inspection is exactly what it exists to detect. It is NOT re-stamped here: `stamp-inspection`/`refreeze` would assert an inspection of that file for issues #514/#515/#518 that I did not perform, which is the false-evidence class the north star forbids. Those issues belong to a different goal (this goal's Backlog Recount explicitly does not claim them), so the re-inspection is that owner's. Remediation is `scripts/validate_issue_source_freeze.py` with the receipt's own paths, run BY someone who re-reads the locator.
- Lessons carried forward: (1) DERIVE A MATCHER'S VOCABULARY FROM THE SURFACE THAT PRODUCES THE TEXT, not from the one instance you measured. The verb list was written from a single sentence and missed the word the repo's own authoring reference tells authors to use — so the gate was nearly inert on real ledgers while passing every test. (2) A FLOOR THAT BUILDS A DIAGNOSIS AND DROPS IT IS A FLOOR THAT TEACHES ROUTE-AROUNDS. The reason existed, the payload had no field for it, and the blocking carrier printed nine words. (3) A TEST CAN NAME ONE PROPERTY AND PIN ANOTHER: the span-width test passed on punctuation, leaving the parameter under review with zero pressure. Mutation found none of these — a reviewer did, because they read what the code would MEET rather than what it was tested against. (4) A SLICE'S BLAST RADIUS CROSSES GOALS: slice 2's one-line gate change invalidated another goal's frozen evidence, and the cheap slice gate could not see it.
- Metrics: Host metrics not exposed to this session; no token/time figures claimed.
