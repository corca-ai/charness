...
- Rows 6-13 have had BOTH bounded rounds and neither was clean, which is now the pattern
  rather than the exception. Round 1 found the read-site rationale REPUBLISHED after this
  slice had already struck it once for rows 1-5, and an undeclared collateral change to
  two planners outside the rows. Round 2 found the round-1 narrowing landed in ONE of five
  surfaces while the record's machine-read `Call sites unproven: none` still resolved
...
### Slice 6: Slice 5 (rows 6-13) — the scaffold family, the handoff readers, the HITL writer

- Objective: Pay down eight more debt rows in two batches sharing one harm shape each, and hit the two-round review floor the rows trigger. The five artifact scaffolds RELOCATE the artifact they are about to write; the two handoff readers and the HITL sync resolve a path they then read or write.
- Why this approach: Batched by harm shape, with a separate measured base/head pair per FILE rather than one measurement generalized across the batch — the inference this goal exists to stop. The guard sits at each file's read site for positional independence, which is the claim that survived review; the harm-based version of that rationale did not.
- Commits: `d4522a3ca` (five scaffolds), `93acaf159` (in-process coverage of the refusal line), `97dfc881a` (literal paths so the coverage mapper can see the tests), `c95c956a7` (handoff readers + HITL sync), `cb469bc22` (round-1 fold), plus the round-2 fold.
- What changed: Guards in `skills/public/{quality,retro,debug,critique,handoff}/scripts/scaffold_*_artifact.py`, `skills/public/handoff/scripts/{parse_handoff_entries,plan_handoff_run}.py`, `skills/public/hitl/scripts/sync_review_artifact.py`. NEW `charness-artifacts/probe/2026-08-19-scaffold-family-version-refusal.md` and `2026-08-19-handoff-hitl-version-refusal.md`, NEW `tests/quality_gates/test_scaffold_version_refusal.py` and `test_handoff_hitl_version_refusal.py`. `scripts/risk_interrupt_lib.py` became a MULTI-CLASS census row without being paid down.
- Alternatives rejected: Rejected: one measurement generalized across a batch sharing a shape. Rejected: paying down `risk_interrupt_lib` on the strength of its census row — its harm did not reproduce and the reading made the row bigger, so it stays unpaid with a second verdict recorded. Rejected in round 2's fold: keeping `Call sites unproven: none` beside a named-and-untested adapter-reaching path; the test was written instead.
- Targeted verification: Ten separate base/head pairs on the real CLIs, each with both polarity controls. Standing lane 10333 passed; `-m release_only` 102 passed at the batch boundary. Changed-line proof `clean` at each commit AFTER two runs where it was not: `blocked` on five uncovered guard lines, then `no-verdict` on a cross-test contamination, both fixed rather than absorbed. Count chain 32 -> 27 -> 24 verified against each commit's own manifest, not asserted. Both records' `## Source text` blocks verified VERBATIM against `git show dd5b6dee9:`; all eight files verified UNGUARDED at the base each record pins.
- Test duplication pressure: Four new code families, all classified `intentional` with per-member reasoning: whole-module similarity between two records-only scaffolds, the bootstrap alias preamble, a four-member `validator_command` family whose fourth member joins on a different half than the cited precedent (corrected by review), and the read-site guard idiom in both handoff readers.
- Critique: TWO bounded rounds; NEITHER CLEAN. Round 1: the guards themselves held — one adapter call site per file, guard above it, both production importers of `payload_for` covered, no bypass into `size_budget` or `resolve_artifact_path` — but the CLAIMS did not. It found the sentence 'a refusal at the entrypoint would cover one caller' published into eight surfaces and false for three of five scaffolds, which is the same unmeasured harm claim this slice had ALREADY STRUCK ONCE for rows 1-5. It found an UNDECLARED COLLATERAL CHANGE to two files outside the rows: `plan_retro_run` and `plan_debug_run` call the now-guarded `payload_for` before building their envelope, so an unhonored declaration produces a one-line refusal instead of the diagnostic plan they used to emit — and both planners' census reasons said the opposite, one calling a now-guarded file '(unguarded)'. It found the parser-refusal test asserting `A or B` for all five, leaving the published 'two of five refuse with a traceback' count unfalsifiable. And it found `risk_interrupt_lib`'s second class recorded only in a probe record and a commit message while the MANIFEST — the surface the gate reads — still said half of it. ROUND 2 was not clean either, and its finding is the class again at a shallower depth: round 1's narrowing of the explicit-path exemption landed in ONE of five surfaces, and the probe record contradicted itself inside one file — `Call sites unproven: none` resolving `covers_all_call_sites: true` while the record's own body named `<path> --with-issues` as an adapter-reaching path no test covered. Round 2 also found the collateral-change pin weaker than the claim it pinned ('no plan at all' held by two absent tokens rather than an empty stream) and the multi-class row's first verdict still ending with the completeness parenthetical its own second verdict indicts. Round 2 additionally resolved three items the round-1 reviewer had to leave UNVERIFIED for want of `git show`, all clean.
- Off-goal findings: None filed. Recorded: two `.claude/worktrees/agent-*` checkouts sit inside the repo root at old SHAs; the census's SCAN_ROOTS do not reach them but other gates' path globs might. Also recorded: `quality` and `critique` resolvers let a parser refusal's ValueError out, so those two refuse with a raw traceback rather than a rendered verdict — real, named, and a different change than these rows make.
- Lessons carried forward: The strongest lesson is a repeat with a new depth: correcting a claim in MOST of its surfaces is not correcting it. Round 1 struck a refuted sentence in eight places; round 2 found the next narrowing struck in one of five. The cheap detector is to grep the phrase, not the file. Second: a batch that shares a harm shape invites exactly one wrong inference — measure once, assert N — and the guard against it is a separate captured base/head pair per file, which is what made the handoff row's broken control visible. Third: a machine-read field can launder a prose contradiction. `Call sites unproven: none` resolved `covers_all_call_sites: true` in the same document that named an untested adapter-reaching path; the resolver is working as designed and the record was lying to it. Fourth, from the coverage lane rather than a reviewer: a test the mapper cannot SEE is a test the coverage lane does not have — five exercised guard lines reported uncovered because the test file built its paths from f-strings.
- Metrics: Eight rows in two batches. Review cost again exceeded implementation cost: `cb469bc22` plus the round-2 fold changed more surfaces than the eight guards did. Both rounds found blockers that would otherwise have shipped, one of them an undeclared behavior change to two files outside the slice. No host token/time telemetry is claimed.
