...
| #588 | 4, public helper | public dogfood invocation fails gracefully without internal policy files | consumer helper / skill author | no | clean consumer-shaped invocation and error contract | closing — typed policy-absence applicability with an empty matrix for a policy-absent synthetic consumer; present invalid policy stays an error. Behavior confirmed by running the PLUGIN entrypoint (a genuinely separate file) against a temp-dir consumer. Scope correction from the closeout review: the `skills/public/quality/` wrapper is NOT a separate copy — its bootstrap imports the same root module — so this establishes mirror fidelity and consumer-shaped invocation, not independence from the root module. Closing carrier authored 2026-08-13; readback follows push. |
| #589 | 2, quality verdict | a fully applied preset lineage has a reachable clean state | declaration lifecycle verdict / quality operator | yes | applied and missing lineage fixtures with expected result | closing — validator-accepted prescribed lineage reconciles only with every exact declared command; missing, unavailable, and metadata-only states render with focused proof. Behavior confirmed through the live repository planner detail, distinct from the lifecycle fixtures. Scope: the `reconciled` branch is reached by no live repo state and remains fixture-proven. Closing carrier authored 2026-08-13; readback follows push. |
| #590 | 2, CI mutation report | a skipped JS mutation stage is distinguished from a missing report | scheduled workflow diagnostics / CI reader | yes | workflow fixture or CI-shaped parser proof | unproven-defer, partial — defect A (marker/collateral path) is proven locally in both directions and mirrored to the plugin export. Defects B and D live only in the workflow's `Open or update mutation issue` script body, are referenced by ZERO tests and no linter, and that step runs only when the pipeline is already red — so the cited scheduled green cannot exercise them. The shipped consumer templates never received the B/D repair (RUN_OUTCOME / Step outcomes / clampBody counts 0/0/0 in both), so the title's defect remains true of every consumer install. The prior `local-proven` label was too strong: this row's own criterion is "workflow fixture or CI-shaped parser proof" and the workflow-fixture half does not exist. Separately, the cited run's log records `8636 passed` inside the sample step, so the undiagnosed runner-only bytecode failure no longer reproduces — non-reproduction, not diagnosis. HELD BACK from the 2026-08-13 cohort carrier. GitHub OPEN; tracker carrier https://github.com/corca-ai/charness/issues/590#issuecomment-5274365441. |
| #595 | 2, runtime verdict | computed runtime signals affect an exit or typed disposition | runtime budget result / quality operator | yes | signal-to-exit behavior tests | closing — explicit advisory disposition with a named final consumer, and focused proof. Behavior confirmed through the selected `check-runtime-budget` lane against live measurement (108044ms latest vs 105000ms budget), distinct from the 50 unit tests. Closing carrier authored 2026-08-13; readback follows push. |
| #597 | 2, fixture proof | an empty tool-fixture set cannot pass as exercised verification | fixture checker and quality gate caller | yes | empty/nonempty fixtures and gate wiring proof | unproven-defer — the owed second bounded round (never run, though this ledger's goal record and the prior carrier both claimed it had) read the repaired surface and found THE REPAIR CARRIES THE CLASS IT FIXED: the refusal is keyed on the fixture COUNT, not on what was compared. Executed proof — a fixture carrying only the six required provenance fields makes the checker print `Verified 1 quality tool fixture(s) against their captured streams.` and exit 0 having compared zero streams. The lane wiring at run-quality.sh:725 is correct but pinned by no regression test. HELD BACK from the 2026-08-13 cohort carrier. GitHub OPEN; tracker carrier https://github.com/corca-ai/charness/issues/597#issuecomment-5274365619. |
...
| #607 | 5, settlement capability | subprocess inventory classifies conservative settlement risk | standing-test economics inventory / quality reviewer | yes | static callsite fixtures distinguish literal-bounded, unbounded-capture, and syntax-unknown seams without claiming runtime child semantics | unproven-defer — the claim "only literal numeric deadlines yield finite" is FALSE on the JS path, inside the accepted-unreviewed R2 repair no reviewer read. The scanner captures a prefix, so `timeout: 30 * 1000` -> `30` -> present -> finite, as do `timeout: 5 + delay` and `timeout: 0` (which Node treats as no timeout); only the unspaced non-numeric form the R2 fixture covers reaches `unknown`. A per-line timeout is also borrowed across sibling calls. Separately, the live inventory run cited as verification CANNOT FAIL (`main()` has only `return 0`), so it was an emission smoke check, not proof of the classification repair. HELD BACK from the 2026-08-13 cohort carrier. GitHub OPEN; tracker carrier https://github.com/corca-ai/charness/issues/607#issuecomment-5274365795. |
