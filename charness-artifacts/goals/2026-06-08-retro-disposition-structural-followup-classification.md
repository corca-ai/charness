...
### Slice 2: Align destination vocabulary into one shared source (anti-drift)

- Objective: Make the retro waste-sibling-scan + the shared retro-issue-destination-split reference and the achieve disposition review use ONE structural-follow-up destination vocabulary, not two drifting copies (prose-only; verdicts unchanged)
- Why this approach: the shared `retro-issue-destination-split.md` is already the documented single source that retro/achieve/issue point at, so the structural-follow-up destination vocabulary lands there once; retro `SKILL.md` is at the 200/200 ceiling (zero headroom — the recent-lessons SKILL.md-ceiling trap), so the prose lands in the two reference files + a one-line pointer in achieve `lifecycle.md`, not in SKILL.md core.
- Commits: (this commit)
- What changed: `retro-issue-destination-split.md` (+mirror) — new "Structural-Follow-Up Destination" section (the four forms as single source, composing with Axis 1/2, lifecycle.md inline named a display copy); `waste-sibling-scan.md` (+mirror) — decision→destination mapping (`same waste, fix now`→`applied:`; `valid follow-up`→`issue`/`repo-local guard:`; `diagnostic-only`/`intentional boundary`→`none —`), explicitly "not a second floor here"; `lifecycle.md` (+mirror) — single-source pointer in the rung-1e bullet.
- Alternatives rejected: (a) edit retro SKILL.md to name the destination — blocked by the 200/200 core ceiling; the reference it already points at is the right home. (b) markdown links from the shared ref into `skills/public/...` — break in the flattened `plugins/` mirror (public→skills); used bare basenames (the shared ref's existing convention) + relative cross-package backticks instead. (c) a retro-side validator floor — out of scope (verdicts-unchanged); the destination stays the disposition review's job.
- Targeted verification: doc-links/check-markdown/validate_skills/skill_ergonomics PASS; mirror byte-synced; verdicts-unchanged proof: 41 validator tests (issue proposal-fields, retro memory/sibling) green, no Python/validator file in the slice diff; no bare issue anchors introduced.
- Test duplication pressure: n/a — prose-only slice, no tests added.
- Critique: fresh-eye Explore reviewer (read-only) — verdict SHIP-WITH-FIXES, no blockers; findings 5/6/7 verified PASS (cross-ref paths robust in the flattened mirror, verdicts-unchanged boundary held — no validator in the diff, no over-claim/new-floor implied). 3 NITs (all clarity, shared ref): (1) name the per-issue Axis-2 `Destination:` independent of sibling-location decisions; (2) clarify `repo-local guard:` is applied-now at a local guard surface; (3) name lifecycle.md's inline form list a display copy, not a second source — all folded.
- Off-goal findings: none.
- Lessons carried forward: `check_doc_links` flags backticked REPO-ROOT-relative paths that resolve to a tracked file as "pathy" (must be markdown links), but markdown links from a `skills/shared/` ref into `skills/public/...` break in the flattened `plugins/charness/` mirror — use bare basenames (no slash → not pathy) for cross-skill code refs in shared references, matching the shared ref's existing convention.
- Metrics: host log probe deferred to bundle boundary (Slice 3).
