...
- Current slice: next is 5 (`#563`, a DELETION). Slices 1, 1b, 2, 3, 3b and 4 are DONE.
...
- **Do not let an INFERRED signal suppress a warning.** Slice 4's first design
  inferred a call-site mutant from removed calls and let that silence the very
  finding the feature exists to surface. Declaration-plus-verification replaced it:
  the author states intent, the tool checks the edit, and it has teeth only where it
  can establish the fact.
- **THREE consecutive slices have had their premise refuted by one command**, and slice
  3b's died completely: `#530`'s version half was already shipped (16/16 resolvers refuse
  an unsupported version), and the unknown-key half was already armed. The slice became
  the defect that measuring exposed instead — the armed tier warned on CORRECT keys in
  every consumer repo, because it scans the CONSUMER's tree for readers that live in the
  installed plugin. Measure the premise before shaping; it has now paid three times.
- **A false-positive rate is a fact about the tree it was measured in.** The tier refused
  to arm `reader-elsewhere` at 13% for being a wolf-crier, then shipped `unknown` at ~100%
  in consumer repos. Same code, unmeasured population.
...
### Slice 4: the warn tier warned loudest where it could see least

- Objective: Slice 3b as planned was `#530`'s version half: replace 16 hand-copied `version` blocks with one shared contract check, and refuse an unknown key rather than drop it. MEASURE THE PREMISE FIRST, per this goal's own highest-yield habit.
- Why this approach: The premise was REFUTED, and this time completely. `grep -rl 'version must be an integer' scripts skills` returns 0; `SUPPORTED_ADAPTER_VERSION` and `validate_adapter_version` already live in `scripts/adapter_lib.py:418`; a live probe of all 16 `skills/public/*/scripts/resolve_adapter.py` against a `version: 7` fixture showed 16/16 REFUSING and 0/16 echoing it back. The unknown-key half was already armed via `adapter_key_registry.unreconciled_keys` -> `validate_adapters.py` -> `run-quality.sh:678`. The folded goal had shipped it; the handoff pointed at work already done. Third consecutive slice whose premise died to one command.
- Commits:
- What changed: `scripts/adapter_warn_tier.py` (NEW: `WarnTierResult`, `reader_corpus_established`, `unestablished_corpus_reason`, `unreconciled_keys`, `_load_yaml_file_report`), split out of `scripts/adapter_key_registry.py` when it crossed the 480-line cap; `scripts/validate_adapters.py` (prints the uninterpreted channel and an explicit did-not-run reason; summary line distinguishes `0 unreconciled` from `not reconciled`); `tests/quality_gates/test_adapter_key_warn_tier.py` (+`establish_reader_corpus`, 5 new tests, 48 total); generated mirrors re-synced and `cmp`-verified byte-identical.
- Alternatives rejected: REJECTED: resolve readers against the installed plugin root so a consumer gets real key verdicts. That is a new capability, not a refusal to claim, and it fights `_is_reader_file`'s deliberate `plugins` exclusion. REJECTED: fold `#574`'s version-reader gaps in here -- a different surface with the opposite failure direction (fail-open on a trust boundary vs. cry-wolf), so combining them would couple two independently-owned surfaces against P2. Filed instead.
- Targeted verification: 48 tests pass. Mutation sweep via `scripts/mutate_and_restore.py`, stated baseline 47 passing: 11 mutants, 11 killed, 0 survived, including TWO CALL-SITE deletions and a mutant reverting `reader_corpus_established` to round 1's rejected proxy. Executed behaviour: `--repo-root plugins/charness` went 126 false WARNINGs -> 0; a consumer fixture declaring three CORRECT keys went 3 -> 0; an over-indented key went silent -> reported; charness itself unchanged at `0 unreconciled declared key(s) across 37 declaring file(s)`; a planted typo key in `.agents/quality-adapter.yaml` still WARNS, so the tier is still armed. `bash scripts/run-quality.sh` exit 0, 88 passed 0 failed.
- Test duplication pressure: dup-ratchet clean (`fixable_ceiling=0 <= floor_F=0`) after the split; no new fixable-eligible families. The split itself was the duplication risk -- prose describing the arming was moved rather than copied, and the registry docstring now points at the tier module instead of restating it.
- Critique: TWO delegated bounded rounds, both DEFECTIVE, and round 2's blocker was inside round 1's repair. Round 1: the guard tested `(repo_root / SHARED_CORE_OWNER).is_file()`, a PROXY for corpus membership that disagrees with the corpus builder -- `_is_reader_file` drops any path containing `plugins`, so under `--repo-root plugins/charness` the shared core is `is_file()` while every reader is excluded. Executed: 126 false WARNINGs over 19 correct shipped examples, reported as a confident count. The repair had reproduced the defect it removed, at LARGER scale, on the layout closest to what consumers install; `test_exported_validate_adapters_runs_from_flattened_layout` runs that exact root and asserts only the exit code, so the suite could not see it. Round 2, reading the repairs: the new did-not-run MESSAGE said the shared core `is not readable`, which is FALSE at that root -- the file is present and packaging REQUIRES it; it is EXCLUDED. An operator acting on it would hunt a missing file that is not missing, and the sentence lived in a different module from the predicate it described (a second declaration, this module's own named anti-pattern). Also caught: `reader_corpus_established` is necessary but NOT sufficient and the docstring claimed otherwise; two library tests asserted `.findings == []` without asserting `scope_established`, so they would pass vacuously if the guard ever regressed; and a `16` that contradicted the `19` two lines below it. Both fingerprint windows verified clean, parent-attributed, BEFORE repairs were applied.
- Off-goal findings: Filed **#574** under the standing approval: three adapter readers OUTSIDE the 16-resolver glob still accept any `version`. `scripts/capability_catalog_sources.py:206` has no version check at all and honored `allow_external_registry: true` and `trusted_skill_roots: ['/etc']` from a `version: 7` fixture while returning `valid: true, errors: []`; it backs the `charness catalog list` CLI that CLAUDE.md tells sessions to run. `skills/public/setup/scripts/setup_adapter.py` publishes `adapter.valid: true` for the same file `setup/resolve_adapter.py` calls `valid: false`. Two handoff consumers compute the verdict and discard it. All reproduced by execution, not inference.
- Lessons carried forward: A false-positive rate is a fact about the TREE it was measured in. This module refused to arm `reader-elsewhere` at a measured 13% for being a wolf-crier, then shipped `unknown` at ~100% in consumer repos -- because the rate was measured only where it was low, and the arming was justified by a population nobody ran it against. Second: my own 16-resolver probe was this goal's class committed by me -- I rendered a verdict from a population that was not the reader set, and only a delegated reviewer caught it. Third, mechanical and expensive: an interrupted mutation sweep leaves the tree MUTATED (#573 again). The 2-minute timeout killed the sweep and left mutant 5 in place; a per-site residue check over the whole plan found it, and `git status` alone would not have -- the file was already legitimately modified.
- Metrics:

### Slice 5: the caller-side question, asked by the tool instead of remembered

- Objective: `#564` re-scoped per this goal's `## Goal`: a repair's proof calls the repaired function directly instead of through the caller that should invoke it, so deleting the CALL SITE leaves the suite green while the repair is dead in production. The issue's filed remedy — a line in the goal template — was DECLINED on P3 grounds by two durable records. Make the question the sweep runner's BEHAVIOUR instead.
- Why this approach: Premise measured first and it HELD, breaking the three-slice streak: `scripts/mutate_and_restore.py` enforces a passing baseline, evidence of a failing test, and restore-on-raise, and nothing about call sites. The baseline-count half of `recent-lessons.md`'s two never-written lines was already tool behaviour; the call-site half was not. No durable record had already decided against a tool-level answer.
- Commits:
- What changed: `scripts/mutate_and_restore.py`: `_called_names` / `removed_calls` (AST call-multiset diff), `MutantResult.declared_call_site` + `removed_calls`, `Sweep.call_site_mutants`, `call_site_non_claim`, a plan-level `"call_site": true` declaration the tool verifies, the counts line gains `N call-site`, the human per-mutant line gains `[call-site; removes ...]`, and `--help` documents the key. `tests/quality_gates/test_mutate_and_restore.py` 36 -> 58 tests. Generated mirror re-synced, `cmp` byte-identical.
- Alternatives rejected: REJECTED — INFER the call-site mutant from removed calls (the first design, killed by round 1). Wrong in both directions and both measured: `return tuple(sorted(x.elements()))` -> `return ()` reports `('elements',)`, so a pure body mutant on a pure helper counted as caller-side proof and SILENCED the warning; a `super().__init__()` deletion reported nothing. REJECTED — REFUSE a sweep with no call-site mutant: the tool cannot see whether one was warranted (a sweep over a constant table has none), so refusing would assert something about the plan it never established. REJECTED — a builtins filter: it was scaffolding for the inference design, and once the declaration replaced it the filter only produced false negatives, refusing an honest declaration whose mutant deleted a `print(...)` call.
- Targeted verification: 58 tests. Mutation sweep via the tool AGAINST ITSELF, stated baseline 57: 13 mutants, 13 killed, 0 survived, 0 refused, 3 declared call-site — including mutants that restore the inference, disarm the false-declaration refusal, revert the non-claim wording, re-silence the empty plan, and stop reading the declaration from the plan. Dogfooded end to end: a body-only plan prints `0 call-site` and the NON-CLAIM on stderr; the self-sweep prints `3 call-site` and no non-claim. `bash scripts/run-quality.sh` — see the closeout run.
- Test duplication pressure: dup-ratchet clean (`fixable_ceiling=0 <= floor_F=0`). The 22 added tests are per-property rather than per-case, which is the duplication shape a sweep-runner suite invites; no new fixable family appeared.
- Critique: TWO delegated bounded rounds, both DEFECTIVE, both blockers inside the repair rather than the diagnosis. Round 1: (a) the INFERENCE design let an incidental removed call suppress the tool's own `#564` warning — `_called_names` keys attribute calls by attribute, so any body mutant dropping a `.join`/`.get`/`.elements` read as caller-side proof; (b) classification sat OUTSIDE the restoring `finally`, re-opening `#573` (an interrupted sweep leaves the tree mutated) through a REPORTING feature, in a module whose docstring promises the restore covers the write; (c) `hasattr(builtins, name)` filtered object-protocol dunders, hiding `super().__init__()` deletions. Round 2, reading the repairs: (a) the non-claim's SENTENCE still said 'no mutant deleted a call site' while the trigger had become 'no mutant was DECLARED' — and the suite already contained a run printing that sentence beside its own `[removes join, str]` line, two contradictory operator-facing statements; (b) a declared call-site mutant that was REFUSED still counted, silencing the non-claim on a mutant that produced no answer; plus a declaration reported `false` for plans that declared `true` on an early-refused mutant, `bool("false")` accepted as a declaration, the same missing stdout flush in the crash arms, and an empty plan staying silent — the most unearned clean report the tool can print. Both fingerprint windows verified clean, parent-attributed, BEFORE repairs were applied.
