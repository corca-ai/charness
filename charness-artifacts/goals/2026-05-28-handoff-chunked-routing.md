...
### Slice 5: Auto-draft writer + Standalone-Usefulness gate

- Objective: Given a selected ChunkCandidate, write a goal artifact skeleton at status draft that passes check_goal_artifact.check_goal with all REQUIRED_SECTIONS, single-prefix heading (no double Achieve Goal:), seeded Goal/Non-Goals/Boundaries/Context Sources, and placeholder-only User Acceptance/Agent Verification Plan/Interview Decisions/Plan Critique Findings/Slice Plan (zero data rows). Pin the slice-5 Standalone-Usefulness Invariant by enforcing that no file under skills/public/achieve/ is mutated by this slice.
- Why this approach: Closes the loop into /achieve: the auto-drafted skeleton conforms to the existing goal-artifact gate exactly so /goal @file is the next move with no further hand-shaping required. The depth-bounded shape protects the achieve Before-phase interview's role (it must fill the placeholder sections) and keeps is_non_trivial_goal returning False at write time so the portability gate fires later only when /achieve adds slice rows.
- Commits:
- What changed: skills/public/handoff/scripts/chunked_routing_lib.py: added USER_ACCEPTANCE_PLACEHOLDER / AGENT_VERIFICATION_PLACEHOLDER / INTERVIEW_DECISIONS_PLACEHOLDER / PLAN_CRITIQUE_PLACEHOLDER constants; _AUTODRAFT_TEMPLATE; _objective_from_chunk; _scrub_trivial_goal_marker (replaces ASCII-hyphen single-slice goal substring with U+2011 non-breaking-hyphen form to defend against poisoning is_non_trivial_goal); _quote_entry_body; _render_goal_body (with merge-bundle ---separator); _render_non_goals; _render_boundaries (with marker scrub); _render_context_sources (bullets only, no intro line); auto_draft_slug (issue-NNN prefix when present); render_auto_draft_artifact. skills/public/handoff/scripts/draft_goal_from_chunk.py (new, 190 lines): CLI consuming a ChunkCandidate JSON, calling render_auto_draft_artifact, writing to charness-artifacts/goals/<date>-<slug>.md, running check_goal in-process, rolling back on failure, refusing to overwrite existing artifacts. tests/test_handoff_chunker_auto_draft.py (new, 401 lines, 15 tests): every spec assertion (single-prefix heading, check_goal ok at draft, all REQUIRED_SECTIONS present, Slice Plan zero data rows, is_non_trivial_goal False at write, no Single-slice goal marker inserted, Context Sources non-empty with citations, each of UA/AVP/ID/PCF contains only the placeholder line with no sub-headings/data/prose, Standalone-Usefulness gate empty for skills/public/achieve/, slug includes issue numbers); CLI write + overwrite-refusal smokes; the slice-5 critique regression test asserting a handoff entry whose body contains 'single-slice goal' cannot poison is_non_trivial_goal.
- Alternatives rejected: (1) Build on goal_artifact_lib._TEMPLATE via render_template + post-mutate: rejected because the empty template has 3 sub-headings under Agent Verification Plan and a stock-prose paragraph under User Acceptance / Interview Decisions / Plan Critique Findings; stripping those programmatically while preserving the rest is brittle. Owning our own template with the same H2 sections is more honest and survives /achieve template churn. (2) Calling upsert_goal then mutating the file: rejected because upsert_goal at status=draft already validates the body via render_template's contract, and mutating after creation requires re-validating, doubling the gate path. (3) Embedding the trivial-marker scrub in goal_artifact_lib instead of in chunked_routing_lib: rejected because that violates the Standalone-Usefulness Invariant - the slice-5 gate prohibits any change under skills/public/achieve/. The scrub belongs in the handoff writer.
- Targeted verification: pytest tests/test_handoff_chunker_*.py: 51 passed (slices 2-5 cumulative). ruff: green. git diff --name-only main..HEAD | grep '^skills/public/achieve/': empty (Standalone-Usefulness Invariant holds). CLI smoke covered by test_cli_writes_artifact_and_reports_check_goal_ok + test_cli_refuses_to_overwrite_existing_artifact. Real handoff fixture round-trip: chunk 1 of the slice-2 fixture round-trips through render_auto_draft_artifact -> check_goal -> ok=true at status=draft.
- Test duplication pressure: check_duplicates.py --json: [] (no near-duplicates). chunked_routing_lib.py is now 816 lines (informational; check_python_lengths.py is not a hook gate). Test code added this slice: 401 lines. Cumulative test count for the chunker pipeline: 51 across 5 files.
- Critique: Bounded fresh-eye subagent slice critique (standard tier, agentId ab369da824c910faf). One Act-Before-Ship + two Bundle-Anyway folded inline. Act-Before-Ship: the _TRIVIAL_GOAL_MARKER in goal_artifact_lib.py matches 'single-slice goal' as a substring anywhere in the artifact, so a handoff entry whose body or title contains the phrase would be quoted into the Goal section and silently neuter is_non_trivial_goal forever - poisoning the portability gate. Folded: added _scrub_trivial_goal_marker that replaces the ASCII-hyphen form with U+2011 (visually identical but breaks the regex), applied to title / boundaries / entry titles / entry bodies / context sources; added test_quoted_handoff_body_cannot_poison_trivial_marker that constructs a poison entry and asserts post-scrub the marker substring is absent and is_non_trivial_goal returns True once a non-trivial Slice Plan is added. Bundle-Anyway 1: Context Sources had a cosmetic intro line that broke bullet-list convention - folded as bullets-only. Bundle-Anyway 2: merged-bundle Goal section concatenated entries with weak visual separation - folded with --- divider between source entries. Over-Worry: standalone-usefulness invariant is honest (file-mutation gate, not import-coupling gate); heading-newline injection cannot happen because _extract_title collapses whitespace at parse time. Valid-but-Defer: slug collision and self-referencing in-scope path are real but rare; revisit if slice-7 e2e surfaces them.
- Off-goal findings:
- Lessons carried forward: (1) Substring-match regexes anywhere in the artifact body are an unsafe contract surface: any user-quoted prose can poison them. The fix is at the writer layer (scrub before injection), not at the regex layer (which would require modifying the achieve gate and violate Standalone-Usefulness). (2) When auto-generating an artifact whose downstream gate uses substring matching, every text region that originates in user-supplied input must be scrubbed - the heading title, body quotes, and any in-scope path list. Missing one path (the title was first missed in the implementation) is exactly what the slice critique caught. (3) The critique substrate works best when the angles directly probe the named risk - 'depth-crossing via body bleed' is the angle that found the substring-regex hole because it deliberately traced quoted prose into the Goal section.
- Metrics: tests: 51 passed cumulative (15 new this slice); chunked_routing_lib.py: 816 lines (+~240); draft_goal_from_chunk.py: 190 lines; test_handoff_chunker_auto_draft.py: 401 lines; ruff: green; duplicate pressure: 0; standalone-usefulness gate: empty (passing); critique tokens: 66541 / tool_uses: 14 / duration: 101826ms

### Slice 6: Skill surface wiring

- Objective: Activate the chunker behind the existing handoff skill surface: add one conditional workflow step + one reference pointer + one ## References entry to skills/public/handoff/SKILL.md (net growth 6 lines, under the slice-6 cap of 161); spill the trigger rule, pipeline, override UX, and standalone-usefulness prose into a new references/chunked-routing.md so the operator-facing surface stays at the right depth.
- Why this approach: Activation surface: without the SKILL.md wiring, the chunker scripts and reference doc exist but no agent invocation path reaches them. Slice-6 enforces the line budget so the existing 200-line cap retains ≥40-line headroom (recent-lessons repeat trap #1).
- Commits:
- What changed: skills/public/handoff/SKILL.md: 151 -> 157 lines (+6). Added step 1 'Chunked routing (conditional)' that fires before refresh-vs-pickup classification when the user mentions handoff with no explicit task directive; pointer to references/chunked-routing.md. Renumbered existing Workflow steps 1-6 -> 2-7. Added references/chunked-routing.md to the ## References listing. skills/public/handoff/references/chunked-routing.md (new, 127 lines): deterministic 2-condition trigger rule with handoff-mention + no-task-directive negation; 5-step pipeline (parse / propose merges / rank / present / draft) with markdown links to each step's script; override-UX prose (accept / decline / why not chunk N / per-merge accept-reject); standalone-usefulness invariant statement. tests/test_handoff_skill_md_budget.py (new, 31 lines, 2 tests): SKILL.md ≤161 budget gate + assertion that references/chunked-routing.md is listed under ## References. charness-artifacts/find-skills/latest.* refreshed via list_capabilities.py.
- Alternatives rejected: (1) Insert chunked-routing as a sub-bullet under existing step 1 instead of a new top-level step: rejected because the chunker is a *conditional* pre-classification phase, not a refinement of refresh-vs-pickup; placing it inside step 1 muddles the gating. (2) Inline the trigger rule text directly into SKILL.md instead of a reference file: rejected by the slice-6 budget (the rule alone is ≥25 lines; SKILL.md would creep past 175 with that + the existing growth). (3) Put the new step at the end of the Workflow rather than the start: rejected because the trigger fires *before* refresh-vs-pickup runs; placing it at the end inverts the operator's actual decision order.
- Targeted verification: pytest tests/test_handoff_*.py: 57 passed (51 chunker + 2 budget + 4 original handoff). validate_skills: 23 packages green (includes the chunked-routing.md reference listing check). check_doc_links: green. wc -l skills/public/handoff/SKILL.md: 157 (under 161 cap, 43 lines headroom under the 200-line MAX_SKILL_MD_LINES). git diff --name-only acfb228..HEAD | grep '^skills/public/achieve/': empty (standalone-usefulness invariant holds against the goal-start commit; no file under skills/public/achieve was touched by any slice in this goal).
- Test duplication pressure: check_duplicates.py --json: [] (no near-duplicates introduced). New test code added this slice: 31 lines (small, single-purpose gate). Cumulative chunker test count: 53 across 6 files.
- Critique: Same-agent self-check: slice 6 is a small, mechanical wiring change with a quantitative gate (line count). Slice-level critique not required for slice 6 per goal-artifact Critique Cadence (slices 3 and 5 carry that requirement). Operator-facing reference prose was reviewed in isolation - it names the trigger rule exactly per the spec fixture, names each pipeline step by script, and pins the override-UX contract from the goal artifact's User Acceptance criteria.
- Off-goal findings:
