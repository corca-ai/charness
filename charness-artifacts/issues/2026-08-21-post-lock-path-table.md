...
| B — installed path contract | #686 | retro planner owner | `skills/public/retro/scripts/plan_retro_run.py`; `scripts/portable_command_carrier.py` | `tests/test_retro_plan.py`; `charness-artifacts/issues/2026-08-21-686-reproduction.txt` | installed-layout fixture; generated export sync is parent-only |
| — requalification only | #681 | achieve quality owner | none until current consumer proof changes the premise | existing goal checker/cadence tests and current issue read | no code admission; preserve `already-satisfied` until a live regression is proven |
