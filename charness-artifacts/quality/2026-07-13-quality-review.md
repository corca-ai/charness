...
- scenario-registry review: representative `setup`, `quality`, `issue`, `impl`, and `handoff` entries in `evals/cautilus/scenarios.json` plus their consumer contracts in `docs/public-skill-dogfood.json` were inspected. Their prompts, expected skills, artifact paths, and acceptance boundaries remain valid after the capability-catalog/session-routing replacement, so no maintained scenario mutation is warranted. Older dated `observed_evidence` sentences that describe the then-current `find-skills` bootstrap remain historical evidence, not live routing instructions. Evidence: commands reading both registries and deterministic `run-evals`/public-skill validation passes.
