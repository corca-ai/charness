...
## Deferred
- Release diff, broken real-host config, previous-tag base-ref lookup/fetch,
  post-create verification, mutation sample-manifest, critique artifact
  discovery, and secret-scan staging suppression are now fail-closed with
  explicit proof.
- Next sibling: continue completion audit over remaining bug-pattern surfaces.
