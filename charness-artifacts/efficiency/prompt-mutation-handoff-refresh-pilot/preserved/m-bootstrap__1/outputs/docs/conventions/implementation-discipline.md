...
- On the landing day of a grandfathered `RULE_DATE`-style floor (a validator
  rule whose enforcement starts at a recorded future date), run the affected
  suite as-of the next enforcement date before pushing, so truncating consumers
  detonate locally instead of on the flip date. Promote this practice to a gate
  only on recorded recurrence (retro 2026-07-08; Floor-Addition Restraint
  applies).
