...
### D38. `check-test-production-ratio` advisory posture is unpinned in run-quality.sh

- Question: Should a gate-plan test pin the `--advisory` flag on the
  `check-test-production-ratio` queue line in
  [run-quality.sh](../scripts/run-quality.sh), so an edit cannot silently flip
  that check between advisory and hard-block?
- Current choice: Defer — no pin (carried from the #420 close, spilled here
  from the handoff `Discuss` queue on 2026-07-09). The posture decision is
  recorded here; the flag is one grep away.
- Why now: A pin on a single CLI flag is new floor weight
  ([Floor-Addition Restraint](./conventions/implementation-discipline.md)) with
  no recorded regression on this queue line.
- Impact surfaces: [scripts/run-quality.sh](../scripts/run-quality.sh),
  [scripts/check_test_production_ratio.py](../scripts/check_test_production_ratio.py),
  [tests/quality_gates/test_quality_runner.py](../tests/quality_gates/test_quality_runner.py).
- Reopen trigger: The advisory/hard-block posture of this check regresses (the
  flag changes without a decision record), or gate-plan pins are added to
  [test_quality_runner.py](../tests/quality_gates/test_quality_runner.py) for
  another reason.
