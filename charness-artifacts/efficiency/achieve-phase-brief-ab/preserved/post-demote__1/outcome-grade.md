...
| auditable-goal-substance | judge | pass | The produced goal artifact turns the prose intent into an auditable goal: it carries reviewable slices, keeps verification visible during the run, and proves the goal with honest non-claims (not an aspirational restatement of the prompt). | Artifact has slice sequence (inventory→helper→ratchet→waves), verification cadence/gate cadence sections, honest non-claims ('nothing has executed', shell scripts NOT all thin wrappers), passed check_goal_artifact ok:true. |
