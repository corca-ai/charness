...
| auditable-goal-substance | judge | pass | The produced goal artifact turns the prose intent into an auditable goal: it carries reviewable slices, keeps verification visible during the run, and proves the goal with honest non-claims (not an aspirational restatement of the prompt). | Goal file has measured scope (144 CLIs, 74 lacking --json), batched slices w/ slice-review packet, verification/gate cadence in Active Operating Frame, and explicit non-claims (no provider/live proof, frozen shapes). |
