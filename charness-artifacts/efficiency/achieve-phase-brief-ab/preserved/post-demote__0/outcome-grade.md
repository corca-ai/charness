...
| auditable-goal-substance | judge | pass | The produced goal artifact turns the prose intent into an auditable goal: it carries reviewable slices, keeps verification visible during the run, and proves the goal with honest non-claims (not an aspirational restatement of the prompt). | Artifact has a Slice Plan table with per-slice objectives/evidence, Verification cadence + Agent Verification Plan sections, explicit non-claim ('no provider, live, remote-CI... proof'), and a measured baseline (270 files, 146 CLI, 78 --json) rather than prompt restatement; check_goal_artifact.py -- |
