...
- **Residual inherited from [#469](https://github.com/corca-ai/charness/issues/469), 2026-08-02.** #469 was closed on its DISCLOSURE half only; its refusal half lands here, because this is the entry that owns "which pre-landing lane pays a toll", and #469 named D45 by mistake (D45 is the CI/local parity gate — a different gate entirely; the misattribution came from D45's own text calling its call "the same class of call as D40" and was caught by the pre-close bounded review).
  - What is now TRUE: [prepush_focused_changed_line_coverage.py](../scripts/prepush_focused_changed_line_coverage.py) does have teeth — it exits 1 on a mapped file with uncovered changed lines, and exits 1 under `--refuse-unestablished`. It refused a real push on 2026-08-02.
  ...
  - **RESOLVED IN PART, operator decision 2026-08-06 ([#488](https://github.com/corca-ai/charness/issues/488)).** The residual assumed two options and the toll question was between them: arm the teeth and risk a permanently-red lane, or leave the green. A third was taken. The verdict now has a THIRD state — `PARTIAL_EXIT = 4` in [check_changed_line_mutation_coverage.py](../scripts/check_changed_line_mutation_coverage.py), `PARTIAL_STATUS` in [prepush_focused_changed_line_coverage.py](../scripts/prepush_focused_changed_line_coverage.py) — which `run-quality.sh` renders UNPROVEN and which `--refuse-unestablished` deliberately does NOT reach. So the false green is gone and **no toll was paid**: policy (a) is intact, nothing newly refuses, and the population measurement this entry demanded before ARMING is not owed, because nothing was armed. Nothing stopped refusing either, and that direction needed a repair to hold: the first cut ordered the new `partial` byte ABOVE the dirty-pool `unestablished` one, which is REFUSABLE at push time -- so a dirty pool that also had a limited scope silently stopped blocking a push. Caught by the bounded review; `unestablished` now wins whenever both hold, guarded at the consumer AND the wrapper, with a regression test at each end.
...
- **Executed measurement replacing the hand counts (2026-08-01).** New script
  ...
  inventory): the presence-only mention total reproduces **170** exactly, **162** of those
  clear today's residual floor, **133** carry a value marker and **29** do not, and a
  ...
  overlap: 116 backticked, 81 `field=`, 24 `field:`. With `--recursive`, which reaches the
  `history/` directory the sibling script's non-recursive glob silently excludes: 132
  artifacts (243 presence-only), 235 floor-clearing mentions, 197 marked, 38 unmarked, 5
...
- Non-claims: the floor as shipped refuses a stub, not a lie, and not incidental prose
  ...
  mentions that clear TODAY's residual floor (162), while the presence-only population is
  170 — both are reported, and the marker split is measured over the 162 only, so the 29
