...
### D28. Template-First Fill Guards And Report-All For Sibling Artifact Validators — REOPENED AND PARTLY RESOLVED (2026-07-27)
...
- Current choice: **The report-all half is resolved: handoff, retro, and ideation now report every violation in one pass BY DEFAULT** (`--fail-fast` opts back into stop-at-first), via the shared `run_validation_checks` plus new `validate_each_artifact` / `run_changed_artifact_validator` helpers in [artifact_validator.py](../scripts/artifact_validator.py). Because one-pass is the default, no `run-quality.sh` flag change was needed. The fill-guard and `--write` halves stay deferred on their original reasoning. Critique and debug keep their opt-in `--report-all` default, so the family is now split on FLAG POLARITY — see the reopen trigger below.
- Why now: **The original "report-all there is ceremony" premise was refuted by measurement, which is exactly this entry's reopen trigger.** The [lesson-recurrence retro](../charness-artifacts/retro/2026-07-26-lesson-recurrence-mechanism.md) measured ~8 validator rounds on a single handoff artifact — the one-rule-per-run report manufactured the retry loop that a separate checked-in lesson ("a counted limit is a planning input, not a retry loop") blames on the reader. Cost is not the rule count; it is that each round is a full gate run and the author re-reads the artifact each time. Handoff has the smallest rule count of the three and produced the largest observed rework, so rule count was the wrong proxy.
...
- Reopen trigger: **Now the polarity split** — `validate_debug_artifact.py` and `validate_critique_artifacts.py` still default to stop-at-first behind an opt-in `--report-all`, while handoff/retro/ideation default to one-pass behind an opt-out `--fail-fast`. Two validators with opposite defaults AND opposite flag names is a trap for the next author. Resolving it means flipping debug/critique to default-on, which reverses the explicit operator narrowing recorded in `a930cc5f`, so it needs an operator decision rather than an agent's judgment. Also still open: `emit_payload_main --write`, and fill guards for any family that accumulates observed n-fold rework evidence.
