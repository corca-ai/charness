...
- Current choice: Defer. Surfaced by the PR #419 adversarial verification and confirmed by an independent verifier: the floor enforces the *presence* of a typed verification record, not its *independence*, so a same-observer `confirmed` passes. The code openly frames itself as a presence/typed-disposition floor, so this is a disclosed residual, not a regression. Tightening it changes what passes at an irreversible boundary and needs its own critique cycle (likely an adapter-contract seam for the independent readback), not a pre-release ride-along.
...
- Current choice: Defer. Surfaced by the PR #419 adversarial verification (independent verifier: minor, disclosed): `gh release view v1`, `gh release view v1 --json url`, and `gh  release view v1` all match, so a same-proxy probe can be recorded as the distinct-channel confirmation. Tightening the shlex match risks rejecting legitimate probe forms and is a boundary-semantics change deserving its own cycle, not a pre-release edit.
