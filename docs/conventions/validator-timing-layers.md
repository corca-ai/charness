...
| inventory-skill-script-references | broad only | stays (advisory) | always exits 0 — emits `WARN:` lines for skill prose whose documented script path does not resolve, never a blocking verdict, so there is no gate-fail to shift left. Same class as `check-public-doc-coupling` above. The blocking half of this repair lives in [test_skill_script_references.py](../../tests/test_skill_script_references.py), which fails when an authoring-layout reference stops resolving; the non-blocking posture here is the operator's Floor-Addition Restraint call (2026-08-02) on a first finding rather than a recorded recurrence. Degrades to a "nothing was checked" notice when no skill packages are present (consumer/tmp repo) |
