...
| validate-public-skill-validation, validate-public-skill-dogfood, validate-cautilus-call-provenance, validate-cautilus-proof, validate-cautilus-scenarios | broad only | stays | policy/proof sweeps over standing artifacts (validate-all class; the three cautilus checks are the "cautilus trio") |
...
| check-secrets, check-supply-chain, check-supply-chain-online, check-shell, check-github-actions, check-links-internal, check-links-external, check-doc-near-duplicates, check-spec-evidence-durability, check-references-link-inventory, check-seed-fixture-budget, check-test-completeness, check-test-production-ratio, check-current-pointer-writes | broad only | stays | repo-wide scans, network, or suite-metric checks — boundary class |
| pytest, check-coverage, specdown, measure-startup-probes, check-runtime-budget, inventory-cli-ergonomics, inventory-gitignore-scan-hygiene, inventory-nose-clones, inventory-sloc, inventory-testability-surface, inventory-ubiquitous-language | broad only | stays | expensive / measuring / inventory class — the boundary IS their timing |
