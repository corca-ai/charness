...
- Uses installed skill metadata and model judgment for ordinary requests; an active objective resumes from the exact `/goal #<parent>` and parent cursor — observation: [`<repo-root>/AGENTS.md`](../AGENTS.md) and [docs/goal-lifecycle.md](./goal-lifecycle.md) define this path.
...
- Uses the owning skill's risk-based review path when a change needs an independent observer; ordinary work does not wait on a standing review ritual — observation: [skills/public/critique/SKILL.md](../skills/public/critique/SKILL.md) scopes critique to material authority, durability, external-write, security, release, compatibility, deletion, and proof-surface risk.
...
- Closes deferred follow-ups proactively when their blocking dependency lands instead of letting them rot — observation: [docs/deferred-decisions.md](./deferred-decisions.md) records the reopen triggers and current disposition.
