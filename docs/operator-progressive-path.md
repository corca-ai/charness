...
- Uses installed skill metadata and model judgment at session start; when hidden inventory is unclear, consult `charness catalog list --repo-root <repo> --json`, then continues active work from [docs/handoff.md](./handoff.md) — observation: [`<repo-root>/AGENTS.md`](../AGENTS.md) `Start Here` and [docs/handoff.md](./handoff.md) `Workflow Trigger` name the pickup path.
