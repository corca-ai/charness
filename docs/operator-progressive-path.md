...
- Uses installed skill metadata and model judgment at session start; when hidden inventory is unclear, consult `charness catalog list --repo-root <repo>`, then continues active work from [docs/handoff.md](./handoff.md) — observation: [`<repo-root>/AGENTS.md`](../AGENTS.md) `Start Here` and [docs/handoff.md](./handoff.md) `Workflow Trigger` name the pickup path.
...
- Treats bounded fresh-eye subagent review as already delegated and does not wait for a second user message when the host permits spawning — observation: [`<repo-root>/AGENTS.md`](../AGENTS.md) `Subagent Delegation` records the repo owner's standing request, named scopes, concrete host-block reporting, and no same-agent substitute; the policy is mirrored in [skills/public/setup/references/agent-docs-policy.md](../skills/public/setup/references/agent-docs-policy.md), with higher-priority instructions remaining authoritative.
...
- Calls `charness worktree doctor` as a non-fatal probe before isolated `impl`/`hitl` on a worktree — observation: [docs/worktree-prepare.md](./worktree-prepare.md) names the consumer-side seed flow and [scripts/worktree_doctor_lib.py](../scripts/worktree_doctor_lib.py) ships the readiness check.
...
- Proposes quality moves and runtime-phase changes when unbudgeted hot spots accumulate, instead of treating gates as fixed — observation: [charness-artifacts/quality/latest.md](../charness-artifacts/quality/latest.md) now writes `Recommended Next Quality Moves` for active candidates, with legacy `Recommended Next Gates` artifacts accepted only as compatibility input.
