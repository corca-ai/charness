...
- Local `main` also contains two unpushed argparse-help packages: web-fetch
  cleared 18 findings and retro cleared 11, each with focused executable proof,
  fresh-eye review, and packaged-mirror synchronization.
...
1. Keep the two follow-up commits local until another push is authorized. If the
...
2. Continue the remaining 51 argparse-help findings one cohesive package at a
   time. The next ranked package is the release planner's eight findings in one
   file; keep focused `--help` proof and do not promote the advisory to a floor.
