...
4. 다음 pickup의 first move는 `find-skills`까지 top-five로 올라왔다는 전제에서, 남은 omitted 후보 중 `handoff-workflow-trigger-followup`를 먼저 올릴지 아니면 `init-repo-partial-normalization-followup`를 당길지 고르는 것이다.
