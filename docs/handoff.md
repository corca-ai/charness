...
- 가장 최근 review proof 기준 control-plane coverage는 `98.0%` (`1196/1221`), test/source ratio는 `0.54` (`10861/20118`), standing pytest는 `306 passed`, eval은 `19` scenario pass, review gate는 `38 passed, 0 failed, 46.7s`다.
...
2. actual quality rerun 기준으로 public-spec layering duplication은 크지 않지만 `specs/index.spec.md`, `specs/tool-doctor.spec.md`는 아직 `no_executable_proof_blocks`로 잡히고, `tool-doctor`는 `implementation_guard_pressure`도 남아 있다. 다음 cleanup 우선순위는 coverage보다 이 public-spec executable proof 약점과 남은 ergonomics advisory다.
3. ergonomics 잔여 항목은 `AGENTS.md`, `README.md`의 command-anchor 기인 option pressure, `UNINSTALL.md`, `init-repo`, `retro`, 그리고 contract snippet 때문에 heuristic이 남아 있는 `spec`이다. entrypoint/skill ergonomics를 hard gate로 올릴지는 아직 결정하지 않았고, 다음 판단은 남은 advisory와 public-spec proof 정리를 더 줄여본 뒤 어떤 rule만 남길지 maintainer가 고르는 순서가 맞다.
