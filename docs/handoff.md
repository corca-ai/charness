...
1. `#19`를 진행한다. `narrative` / `find-skills` / 필요하면 `announcement` bootstrap이 richer truth surface repo에서 너무 얕게 fallback하지 않도록, inferred source-doc / discovery-policy nudge를 한 family로 정리한다.
