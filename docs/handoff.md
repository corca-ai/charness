...
- 다음 세션이 support-tool follow-up이면 먼저 [docs/support-tool-followup.md](support-tool-followup.md)를 읽는다. `create-cli` probe-layer slice, `find-skills` recommendation helper first slice, `quality` installable-CLI probe-contract posture(`#10`)는 landed 상태다. 이 workstream에서 다음 first move는 보통 `quality`가 missing external tool 때문에 deeper proof를 못 할 때 shared recommendation/install contract를 더 직접 재사용하게 만들거나, 그 뒤 follow-on인 executable-spec / fragile-margin pass(`#13`)로 넘어가는 쪽이다. 각 slice마다 public skill source-of-truth 수정과 `charness` dogfood를 함께 끝내야 한다.
...
- 2026-04-14 closeout에서는 `quality` `#10` source-of-truth 변경, dogfood artifact 갱신, checked-in plugin export sync, 그리고 [`skills/public/find-skills/scripts/list_capabilities.py`](../skills/public/find-skills/scripts/list_capabilities.py) file-length gate fix까지 포함해 `./scripts/run-quality.sh`가 다시 초록으로 돌아왔다.
...
0. support-tool follow-up을 이어갈 때는 [docs/support-tool-followup.md](support-tool-followup.md)를 먼저 읽고, 여기서 정한 issue triage와 dogfood acceptance를 그대로 따른다. 특히 `create-cli`, `quality`, `find-skills`/recommendation flow 변경은 모두 skill 수정 + `charness` repo dogfood까지 한 slice로 묶는다.
   - `quality` `#10`은 landed 상태다. 다음 support-tool slice가 이 축을 잇는다면, first move는 quality output이나 repo-owned helper가 shared recommendation/install payload를 직접 재사용하게 만드는지, 아니면 `#13` executable-spec follow-on으로 갈지 먼저 정하는 것이다.
