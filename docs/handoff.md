...
- `find-skills` now actually persists that capability map to
  `charness-artifacts/find-skills/latest.md` and `latest.json`; unchanged
...
- The newest benchmark slice now favors `init-repo`, `handoff`, `find-skills`, `retro`, and `premortem`. The continuation/normalization policy corrections moved into the top-five cap after their reviewed dogfood evidence and packet candidates were refreshed; `quality` and `gather` are currently below the cap alongside `hitl`, `debug`, `release`, and `spec`. For working-tree diffs, do not rely on `--baseline-ref` shorthand alone: prepare an explicit baseline repo and pass `--baseline-repo <baseline> --candidate-repo <current repo>` to [scripts/eval_cautilus_chatbot_compare.py](../scripts/eval_cautilus_chatbot_compare.py), otherwise uncommitted candidate changes are invisible.
...
4. 다음 pickup의 first move는 `handoff`와 `init-repo`까지 top-five로 올라왔다는 전제에서, 남은 omitted 후보 중 `quality-proof-layering-followup`를 다시 당길지 아니면 `gather-official-path-before-browser-followup`를 다시 올릴지 고르는 것이다.
