...
- Upstream `cautilus scenario propose` still defaults to a `5`-proposal cap when the input packet omits `limit`, but the checked-in chatbot packet now sets `limit: 12` so all current long-context candidates stay emitted in the local proposal surface. The runner still records both `candidate_keys` and `omitted_candidate_keys`, but the normal local current pointer should now show the full packet rather than a top-five subset.
- The newest benchmark slice now reflects that explicit-limit override instead of a top-five ranking race: `quality`, `init-repo`, `narrative`, `spec`, `debug`, `release`, and `hitl` all reappear in the emitted proposal set once the packet stops relying on the upstream default cap. For working-tree diffs, do not rely on `--baseline-ref` shorthand alone: prepare an explicit baseline repo and pass `--baseline-repo <baseline> --candidate-repo <current repo>` to [scripts/eval_cautilus_chatbot_compare.py](../scripts/eval_cautilus_chatbot_compare.py), otherwise uncommitted candidate changes are invisible.
...
4. 다음 pickup의 first move는 checked-in chatbot packet의 explicit `limit: 12` override를 유지할지, 아니면 upstream `cautilus`가 `limit`를 help/schema/docs에 더 명시한 뒤 다시 product default에 기대도 되는지 판단하는 것이다. `corca-ai/cautilus#14` tracks the default-cap visibility confusion on the upstream side.
