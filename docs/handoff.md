...
- Continue through `issue` for #433 publication/closeout boundaries and through
  `quality` for the remaining argparse-help debt. Keep reversible cleanup
...
- Local `main` contains the complete #433 repair and quality sweep through
  `c4e92473`: normal and resumed release-close carriers validate through the
  issue-owned consumer before expensive work.
...
- #433 remains OPEN. No release, provider mutation, or issue close was performed
  by the local goal; a push is authorized for this handoff sequence, but issue
  close still requires its own explicit boundary decision.
...
1. Push local `main`, including the #433 repair and this refreshed baton, to
   `origin/main`; then confirm the remote readback matches the pushed commit. If
   the operator separately authorizes issue close, verify the published carrier
   through a distinct channel before closing #433.
2. Start the remaining argparse-help debt as the next local autonomous slice:
   re-inventory current findings, rank one cohesive package, and clear that
   package with focused `--help` proof. Do not turn the advisory into a new
   blocking floor or sweep all sites without a triage lock.
...
- Closing #433, publishing another release, and applying provider changes remain
  separate irreversible approvals; the authorized push does not imply them.
