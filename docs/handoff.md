...
- **The `#514/#515/#518` freeze was re-stamped for the SIXTH time, and the first WITH a recorded basis.** Two sentences turned a mechanical reflex into a reviewable decision, which is the entire content of `#562`'s proposal. `#562` is claimed by the successor, with the budget planned rather than borrowed.
- **Two operator decisions are queued and block nothing:** the direction for `#562` (delete the locator pin, or require a basis per re-stamp), and the renderer-versus-reference spelling split in `setup` — the renderer is gated against baking a model id into the AGENTS.md contract while `default-surfaces.md` instructs an agent to write exactly that.
