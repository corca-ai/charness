...
- 2026-04-18 UTC dogfood proof: `cautilus instruction-surface test --repo-root .` now completes with `recommendation=accept-now`, `4 passed / 0 failed / 0 blocked`. The checked-in bootstrap case routes to `find-skills`, the compact no-bootstrap implementation case routes directly to `impl`, and the expanded no-bootstrap contract-shaping case routes directly to `spec`.
- The compact contract-shaping case is intentionally observational rather than a hard expectation. Current dogfood shows compact surface still allows direct `spec` selection in practice, so that behavior is now evidence to watch instead of a brittle failing assertion.
...
4. `cautilus` instruction-surface wiring 자체는 이제 닫혔다. 다음 pickup은 이 surface를 더 넓힐지 여부다: compact contract-shaping route를 계속 observational로 둘지, 아니면 additional cases/prompts로 더 좁은 behavioral expectation을 만들지 결정한다.
