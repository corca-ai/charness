...
- The newest benchmark slice now favors `retro`, `gather`, `quality`, `init-repo`, and `handoff`. Refreshing the reviewed `retro` structural-cause seam pulled it back into the top-five cap and pushed `find-skills` below the line; `premortem` also remains below the cap alongside `find-skills`, `hitl`, `debug`, `release`, and `spec`. For working-tree diffs, do not rely on `--baseline-ref` shorthand alone: prepare an explicit baseline repo and pass `--baseline-repo <baseline> --candidate-repo <current repo>` to [scripts/eval_cautilus_chatbot_compare.py](../scripts/eval_cautilus_chatbot_compare.py), otherwise uncommitted candidate changes are invisible.
...
4. 다음 pickup의 first move는 `retro`, `gather`, `quality`가 top-five에 들어간 전제에서, 남은 omitted 후보 중 `find-skills-canonical-artifact-followup`를 다시 올릴지 아니면 `premortem-canonical-subagent-followup`를 다시 당길지 고르는 것이다.
