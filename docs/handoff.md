...
- 다음 세션이 support-tool follow-up이면 먼저 [docs/support-tool-followup.md](support-tool-followup.md)를 읽는다. `create-cli` probe-layer slice, `find-skills` recommendation helper first slice, `quality` installable-CLI probe-contract posture(`#10`), `quality` executable-spec / fragile-margin pass(`#13`)는 landed 상태다. 이 workstream에서 다음 first move는 보통 `quality` output이나 shared repo-owned helper가 recommendation/install payload를 더 직접 재사용하게 만들거나, 그 다음에야 `init-repo` follow-on(`#11`)을 다시 볼지 정하는 쪽이다. 각 slice마다 public skill source-of-truth 수정과 `charness` dogfood를 함께 끝내야 한다.
...
- 다른 머신 Codex dogfood 추가 관찰: `charness update` 뒤 restart만으로는 cache refresh가 되지 않았다. 반면 `charness reset && charness init` 뒤에는 이제 `init`도 Codex CLI가 있으면 official app-server `plugin/install` 경로를 먼저 시도해 baseline을 자동으로 만들려고 한다. 현재 `main`의 `charness init`/`update`는 모두 same `plugin/install` seam을 타지만, 남은 proof는 "clean installed baseline에서 upstream-visible payload delta를 만든 뒤 `charness update`만으로 cache가 맞고, restart 뒤 visible skill surface까지 따라오는지"이며, 실패 시 `re-enable`과 `reinstall`을 순서대로 분리 기록해야 한다.
...
- 2026-04-14 closeout에서는 `quality` `#10`, `#13` source-of-truth 변경, dogfood artifact 갱신, checked-in plugin export sync, 그리고 [`skills/public/find-skills/scripts/list_capabilities.py`](../skills/public/find-skills/scripts/list_capabilities.py) file-length gate fix까지 포함해 `./scripts/run-quality.sh`가 다시 초록으로 돌아왔다.
...
0. support-tool follow-up을 이어갈 때는 [docs/support-tool-followup.md](support-tool-followup.md)를 먼저 읽고, 여기서 정한 issue triage와 dogfood acceptance를 그대로 따른다. 특히 `create-cli`, `quality`, `find-skills`/recommendation flow 변경은 모두 skill 수정 + `charness` repo dogfood까지 한 slice로 묶는다.
   - `quality` `#10`, `#13`은 landed 상태다. 다음 support-tool slice가 이 축을 잇는다면, first move는 quality output이나 repo-owned helper가 shared recommendation/install payload를 직접 재사용하게 만드는지, 아니면 `init-repo` follow-on(`#11`)을 열 시점인지 먼저 정하는 것이다.
