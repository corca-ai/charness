...
- 2026-04-10에 Claude local plugin surface도 다시 확인했다. `claude --print --plugin-dir /home/ubuntu/charness`는 정상 응답했고, Codex 쪽은 현재 공개 CLI/문서 기준 `--plugin-dir` 대응 플래그가 보이지 않는다.
...
- 같은 날 Codex packaging metadata도 richer discovery surface로 올렸다. checked-in [plugin.json](/home/ubuntu/charness/.codex-plugin/plugin.json)은 이제 `author`, `homepage`, `repository`, `keywords`, `interface` metadata를 포함하고, schema/validator/export smoke도 이를 검증한다.
