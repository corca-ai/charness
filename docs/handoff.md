...
- `gather` now has an explicit browser-mediated private SaaS contract. The public skill and references require official API/export first, then `agent-browser` fallback, and honest remote/headless/auth-bootstrap degradation; the `find-skills` current pointer reflects that surface.
...
4. 다음 pickup의 first move는 새 `gather-official-path-before-browser-followup` 후보를 reviewed dogfood/evidence surface로 승격할지, 아니면 다른 missing long-context correction이 더 우선인지 판단하는 것이다.
