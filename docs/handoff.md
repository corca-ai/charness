...
1. `init-repo`에 대해 greenfield repo 1개와 partial repo 1개 정도의 real dogfood 또는 eval scenario를 추가할지 먼저 결정하고, 최소한 하나는 실제로 돌려 본다.
2. regenerated plugin surface와 root marketplace가 여전히 source of truth와 일치하는지 다시 확인한다.
3. 다른 머신 또는 실제 interactive Codex에서 root `.agents/plugins/marketplace.json` 기준 local marketplace discovery/install visibility를 확인한다.
4. 가능하면 restart 후 same-checkout update model까지 본다.
5. `init-repo` dogfood와 Codex proof 결과를 바로 이 handoff나 별도 durable artifact에 남긴다.
