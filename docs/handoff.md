...
4. `cautilus` instruction-surface wiring은 붙었지만 아직 fully green은 아니다. 다음 pickup의 첫 질문은 checked-in bootstrap case를 hard expectation으로 계속 둘지, 아니면 summary schema를 넓혀 `bootstrap helper`와 `work skill`을 분리할지다. 그 다음 질문이 `mode evaluate`가 prepared compare worktree를 더 직접 소비하도록 charness adapter contract를 넓힐지다.
