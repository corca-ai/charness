...
4. **`awiki` was asked for as a support binary and never installed as one — I pruned that state as stale and it was not.** `integrations/tools/` declares eleven binaries with matching `integrations/locks/` entries; awiki is in NEITHER, and `docs/support-skill-policy.md:37-39` names an integration manifest as exactly where a binary like it belongs. What DOES exist: a [captured fixture](../charness-artifacts/quality/fixtures/awiki-0.5.0-docs-lint.json), the `#518` reconciliation contract, and a TRANSCRIBED prior measurement — `awiki lint -root docs -recursive` returned exit 1 with 40 documents, **7 orphans**, 0 islands, 230 link-only lines, over a scope not re-verified since. the repo's own quality adapter has no awiki entry. Tracked in `#566`, whose original framing was wrong and is corrected in its comments.
5. **Then pick the next goal, in the operator's stated sequence.** After the packet cleanup, use [the open-issue opinion file](../charness-artifacts/audit/2026-08-08-open-issue-opinion.md) as the basis — **but not blindly**; it says so itself, and four of its positions were corrected by the operator within one session. Read `../craken-agents` alongside it, specifically for **awiki usage and doc gardening** (its `docs:lint` is a GRAPH check that fails on orphans and islands, charness has no equivalent, and that repo's own `../craken-agents/docs/documentation.md` states in prose that the check is unenforced and names every hook and workflow that does not run it). craken does NOT install charness, so it is a shape comparison only.
6. **The real consumer repos are `../ceal`, `../cmanki`, `../stdy.blog`, `../journal.stdy.blog`, `../cautilus`** — all five install charness and carry `charness-artifacts/`. Measured this session and load-bearing for `#523`: every one has an `AGENTS.md` of 11.8-18.3 KB and two are LARGER than this repo's 15,806 bytes, so the handbook shape propagates through `setup` rather than being an internal cost. Skill usage, counted by real write paths: `critique`/`quality`/`gather`/`retro`/`debug`/`setup` 5/5, `achieve`/`handoff` 4/5, only `create-cli` at 0/5 — so `#521` has almost nothing to delete on usage grounds. Two scope corrections to carry: `## Subagent Delegation` in `AGENTS.md` is LOAD-BEARING (without it the bounded critique subagent does not run) so `#523` must cut the rest and hold it out; and skill EVALUATION belongs to `../cautilus`, so `#519`/`#520` are a run-cadence question, not new measurement infrastructure.
