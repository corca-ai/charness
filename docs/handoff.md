...
- **Read [recent lessons](../charness-artifacts/retro/recent-lessons.md) BEFORE acting.**
  A headless run proved this line works — and that the digest's 4 trap slots dropped the two
  sharpest lessons, so THIS file and the spec artifact are the channel that carries.
...
- **`#572` CLOSED as `consolidated` into `#590`.** The green cron on `ed90c1f3` is NOT
...
- **Gate green: 89/0, 1 UNPROVEN** — baseline taken BEFORE any deletion. Filed `#596`
  ...
  passes on an empty set, unwired), `#598` (gate blocking on a word preference + a
  five-week-unactioned audit), `#599` (no "what reads this?" command).
- **D53/D54/D55** in [deferred-decisions](./deferred-decisions.md) each name a reopen
  trigger; D53's is NOT in-repo observable.
...
1. **OPERATOR RULING — delete the pickup ambiguity heuristic and both eval specs.** It
   ...
   headless run of this item already happened** (`# Observation run 2026-08-11`); its patch
   is evidence, not a shortcut — re-do the work under verification.
...
5. **Waves 2-3.** Bugs `#539` `#581` `#588` `#528` `#589` `#542`; then `#586` `#590`
   `#593`-`#599` `#550` `#527`. Umbrellas are in no wave until their work ships.
...
- **A headless run of this repo cannot reach its own stop gate**: the contract demands
  gates, the default permission posture blocks `python3`. Grant commands or expect a stop.
...
- **Closing an issue can delete a ruling — or rot its pointer.** `#536` closed COMPLETED
  while the cost it tracked survived.
