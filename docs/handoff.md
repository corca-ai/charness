...
- 다음 세션이 support-tool follow-up이면 먼저 [docs/support-tool-followup.md](support-tool-followup.md)를 읽는다. `create-cli` probe-layer slice, `find-skills` recommendation helper first slice, `quality` installable-CLI probe-contract posture(`#10`), `quality` executable-spec / fragile-margin pass(`#13`), 그리고 shared recommendation/install payload의 `quality` 재사용 seam은 landed 상태다. 이 workstream에서 다음 first move는 보통 `init-repo` follow-on(`#11`)을 이제 열지, 아니면 verifier-selection helper 쪽 follow-on을 먼저 잡을지 정하는 쪽이다. 각 slice마다 public skill source-of-truth 수정과 `charness` dogfood를 함께 끝내야 한다.
...
0. support-tool follow-up을 이어갈 때는 [docs/support-tool-followup.md](support-tool-followup.md)를 먼저 읽고, 여기서 정한 issue triage와 dogfood acceptance를 그대로 따른다. 특히 `create-cli`, `quality`, `find-skills`/recommendation flow 변경은 모두 skill 수정 + `charness` repo dogfood까지 한 slice로 묶는다.
   - `quality` `#10`, `#13`, recommendation/install payload의 `quality` 재사용은 landed 상태다. 다음 support-tool slice가 이 축을 잇는다면, first move는 `init-repo` follow-on(`#11`)을 열 시점인지, 아니면 verifier-selection helper를 먼저 만들지 정하는 것이다.
