...
8. derived memory / smoothing 작업을 이어받으면 [charness-artifacts/spec/derived-memory-smoothing.md](../charness-artifacts/spec/derived-memory-smoothing.md)를 먼저 읽고, quality runtime EWMA는 advisory-only로 유지한다. 다음 후보는 freshness validator 확장, retro digest 기준, 또는 debug seam-risk index 중 실제 소비자가 있는 것만 고른다.
