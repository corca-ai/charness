...
4. 다음 pickup의 first move는 `gather` reviewed dogfood가 이제 강화됐다는 전제에서, 다음 missing long-context correction이 `quality`/`retro`/`premortem` 중 어디인지 고르는 것이다.
