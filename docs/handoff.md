...
- 2026-04-18 UTC dogfood proof is now stricter but not yet fully closed. `cautilus instruction-surface test --repo-root .` currently lands at `recommendation=reject`, with compact no-bootstrap implementation routing directly to `impl`, expanded no-bootstrap contract-shaping routing directly to `spec`, and compact contract-shaping staying observational.
- The remaining gap is the checked-in bootstrap case. Current `cautilus` runs load `find-skills` first but often still report `impl` as the selected work skill, so the present instruction-surface summary contract cannot cleanly distinguish bootstrap-helper selection from durable work-skill selection.
...
4. `cautilus` instruction-surface wiring은 붙었지만 아직 fully green은 아니다. 다음 pickup의 첫 질문은 `corca-ai/cautilus#11`이 제품 쪽에서 bootstrap-helper vs work-skill 분리를 받아갈지 확인하는 것이다. 그 전까지는 checked-in bootstrap case expectation을 local adapter wording만으로 억지로 닫지 않는다. 그 다음 질문이 `mode evaluate`가 prepared compare worktree를 더 직접 소비하도록 charness adapter contract를 넓힐지다.
