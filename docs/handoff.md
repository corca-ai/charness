...
- First read the goal's Active Operating Frame, current quality record, and recent lessons, then live-read #496 through the issue adapter with comments before designing a remedy.
...
- The active goal has independently verified CLOSED slices #468, #503, #506, #502, #507, #511, and #504; 10 planned issues remain: #480, #482, #483, #484, #491, #496, #505, #508, #509, and #510.
...
- The next planned slice is #496: re-read its hollow-refill-report request and shape the smallest honest disposition against the current goal.
...
2. Run the live #496 issue read with comments and bind its hollow-refill-report request to this goal's ownership contract.
...
4. Implement the smallest honest #496 slice, prove it through the issue/closeout workflow, and close it only after its own carrier, critique, distinct behavior channel, remote evidence, and adapter readback satisfy the floor.
5. Append the complete #496 disposition to the goal and refresh this handoff before the next pickup; keep the goal active until all scoped issues are independently dispositioned.
...
- Preserve strict sequence by default. If #496 is externally blocked, record its OPEN state, missing proof, owner/action, and explicit re-rank authorization before moving on.
...
- [Issue #496](https://github.com/corca-ai/charness/issues/496)

- Refresh kept: #504's carrier `5372631a`, remote CI run `30999412722`, CLOSED readback, the ten-issue open set, and #496 as the next action because each changes the next operator's first move; see the [active goal](../charness-artifacts/goals/2026-08-05-close-all-open-issues-generative-sequence.md).
