...
- Session 9 후반에 [package.json](/home/ubuntu/charness/package.json), [.markdownlint-cli2.jsonc](/home/ubuntu/charness/.markdownlint-cli2.jsonc), [.secretlintrc.json](/home/ubuntu/charness/.secretlintrc.json), [check-markdown.sh](/home/ubuntu/charness/scripts/check-markdown.sh), [check-secrets.sh](/home/ubuntu/charness/scripts/check-secrets.sh), [check-shell.sh](/home/ubuntu/charness/scripts/check-shell.sh), [check-links-external.sh](/home/ubuntu/charness/scripts/check-links-external.sh)가 추가돼 markdownlint와 secret gate가 quality runner에 연결됐다.
- Session 9 품질 게이트의 secret scanner는 이제 [gitleaks.toml](/home/ubuntu/charness/.gitleaks.toml) 기반 `gitleaks` 우선, repo-local `secretlint` 폴백으로 정리됐다. `quality` reference는 npm-less repo의 first gate로 `gitleaks`, Node rule-customization이 필요한 repo에는 `secretlint`를 권장한다.
...
- 2026-04-10에 Claude local plugin surface도 다시 확인했다. 다만 처음에는 repo root를 `--plugin-dir` 대상으로 잡아 install은 되지만 skill discovery가 깨지는 packaging bug를 놓쳤고, 이후 checked-in install surface를 [plugins/charness](/home/ubuntu/charness/plugins/charness)로 옮겨 flat `skills/` export를 강제했다. Claude local path는 이제 `/home/ubuntu/charness/plugins`, Codex local path는 root [.agents/plugins/marketplace.json](/home/ubuntu/charness/.agents/plugins/marketplace.json)에서 `./plugins/charness`를 가리키는 구조다.
...
- 같은 날 Codex packaging metadata도 richer discovery surface로 올렸다. checked-in [plugin.json](/home/ubuntu/charness/plugins/charness/.codex-plugin/plugin.json)은 이제 `author`, `homepage`, `repository`, `keywords`, `interface` metadata를 포함하고, schema/validator/export smoke도 이를 검증한다.
...
- `sync-support`의 v1 기본 권장 전략은 이제 `reference`로 고정됐지만, generated reference artifact는 machine-local state로 본다. checked-in install surface와 public plugin bundle에서는 [skills/support/generated/](/home/ubuntu/charness/skills/support/generated/)를 제외하고, 실제 plugin bundle에는 flat public `skills/`와 non-discoverable `support/`만 싣는다.
...
- checked-in install surface는 이제 [plugins/charness](/home/ubuntu/charness/plugins/charness) 아래에 있다. root-level generated artifact는 [marketplace.json](/home/ubuntu/charness/.claude-plugin/marketplace.json)과 [marketplace.json](/home/ubuntu/charness/.agents/plugins/marketplace.json)만 남기고, 실제 host manifests는 [plugins/charness/.claude-plugin/plugin.json](/home/ubuntu/charness/plugins/charness/.claude-plugin/plugin.json), [plugins/charness/.codex-plugin/plugin.json](/home/ubuntu/charness/plugins/charness/.codex-plugin/plugin.json)으로 이동했다. 이 파일들은 [charness.json](/home/ubuntu/charness/packaging/charness.json)에서 [sync_root_plugin_manifests.py](/home/ubuntu/charness/scripts/sync_root_plugin_manifests.py)로 생성되고, [validate-packaging.py](/home/ubuntu/charness/scripts/validate-packaging.py)가 shared packaging manifest와의 일치를 강제한다.
- real Claude gather repro에서 install-proof의 빠진 seam도 드러났고 지금은 고쳐졌다. installed plugin tree는 repo-owned helper [scripts/](/home/ubuntu/charness/scripts/)와 optional [integrations/locks/](/home/ubuntu/charness/integrations/locks/)를 같이 export하고, public helper scripts는 source repo와 installed plugin 둘 다에서 runtime root를 찾는다.
- `find-skills` helper는 source-repo 실행과 installed-plugin 실행을 구분한다. source repo에서는 consumer repo의 local skill tree를 보고, installed plugin에서는 bundle-local public/support surface를 본다.
- vendored [notion-to-md.py](/home/ubuntu/charness/skills/support/gather-notion/vendor/notion-to-md.py)는 이제 Notion `loadPageChunk`의 double-wrapped block payload를 풀고, `syncRecordValues` 반복 fetch로 missing child block을 보강하며, `header`/`sub_header`/`sub_sub_header` children을 재귀한다. placeholder-only output은 exit 0으로 숨기지 않고 실패시킨다.
- repo-owned [check-markdown.sh](/home/ubuntu/charness/scripts/check-markdown.sh)는 이제 tracked markdown만 lint한다. user-provided logs나 host-local `.claude/` artifacts가 repo quality gate를 깨지 않게 했다.
- live smoke로 [plugins/charness/support/gather-notion/scripts/export-page.py](/home/ubuntu/charness/plugins/charness/support/gather-notion/scripts/export-page.py)를 same published Notion URL에 다시 돌렸고, 이전 41-byte placeholder 대신 `skill-outputs/gather/notion-smoke.md`에 9KB+ non-placeholder markdown이 생성되는 것을 확인했다.
...
2. Claude direct-install과 public skill discovery는 확인됐고 root `cautilus` consumer smoke도 이미 끝났지만, 다음 session의 첫 구현 slice는 fresh machine public install proof다. Claude shared install에서는 `gather` -> `gather-notion` runtime까지 다시 확인하고, Codex는 discover/install visibility와 dual-host same-checkout update model을 검증한다.
