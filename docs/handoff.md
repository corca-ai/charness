...
- **SIX goal artifacts claim `Status: active`.** Three are from June and are almost certainly stale; [repair-declaration-to-verdict-at-root](../charness-artifacts/goals/2026-08-07-repair-declaration-to-verdict-at-root.md) and [refuse-the-verdict-a-surface-never-earned](../charness-artifacts/goals/2026-08-09-refuse-the-verdict-a-surface-never-earned.md) are the two live ones and they are the SAME FAMILY — a declaration nothing reconciles, and a verdict nobody established. Decide whether to fold, sequence, or retire before activating anything else. Recount with a `Status:` grep over `charness-artifacts/goals/`.
- **The push is BLOCKED and was not taken.** `check-cli-skill-surface` fails because its `doctor.py` probe needs ~21s against a 20s timeout. Measured on the PRE-SESSION tree too, so it predates this work and is environment speed, not a regression. Raising `CHARNESS_CLI_SKILL_SURFACE_PROBE_TIMEOUT_SECONDS` would loosen a floor to buy a green push, which revokes the push grant — so it was left refusing. This needs an operator call: fix the probe's cost, widen the budget deliberately, or push from a faster machine.
