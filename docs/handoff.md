...
- Then run `## Next Session` item 1 — the premise check over the next slice's scoped
  items — and only then invoke `impl`. S1-S6c are committed. **S6b-2 is next**, and the
  owner deferred the release rather than cutting S6b-2: the tail is `S6b-2 -> S7`.
...
- **S6c is committed** — the export ships its bootstrap dependency contract, the documented
  `gather` entrypoint guards its import, and a detector refuses an unguarded third-party
  import in any script the export tells a consumer to RUN. Its unshipped-path arm is
  ADVISORY by decision, and a test pins that split for a later slice to retract by name.
  Re-prove: `python3 scripts/check_export_self_sufficiency.py --repo-root .`.
- **[#634](https://github.com/corca-ai/charness/issues/634) STAYS OPEN and is commented**
  with the honest score: 2 of ~16 enumerated items. The cwd-relative instruction sites and
  unguarded shell gates are neither repaired nor detectable by what shipped.
- **S6b-1 is committed** — one classifier for coverage instrumentation, the closeout
  ...
  `python3 scripts/run_standing_pytest.py --pytest-target tests/quality_gates/test_coverage_builder_policy_parity.py`.
...
- **S6 is committed** (`git show 54654b032 --no-patch --format=%B`): worktree isolation,
  ...
  [#633](https://github.com/corca-ai/charness/issues/633).
...
or issue closure. S6b-2 and S7 have not started; the owner deferred the release on
2026-08-15 rather than cutting scope. The CI wiring that passes the standing runner to the
changed-line step has NOT run on hosted CI, and the shipped bootstrap contract has NOT been
exercised from an installed plugin on a machine without this repo — both are proven locally
only.
...
2. **S6b-2** (SC14, 15, 16, 17, 19 in the [release scope contract](../charness-artifacts/spec/2026-08-15-6-0-0-release-scope.md)): the rest of cost, which needs S6c's detector first so
