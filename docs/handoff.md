...
- **Item 5 (spec'd + reviewed, slice 1 in progress):** boy-scout dup ratchet.
  ...
  (escalation-ladder policy, tuned in
  [quality-adapter.yaml](../.agents/quality-adapter.yaml), **standalone portable
  gate script** so consumers get it too). Slice 1 DONE so far: `family_id`
  propagation (5226ad9f). NEXT: `dup-review.json` schema + `quality` seed step
  (intentional vs unreviewed) + tests; then slice 2 = the gate script + adapter
  wiring + portability/escalation acceptance tests (also closes critique C5).
