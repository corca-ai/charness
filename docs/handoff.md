...
- 현재 `main`에는 update propagation 실험용 public skill `update-probe`와 package version `0.0.3-update-probe`가 올라가 있다. 다른 머신에서는 clean Codex baseline 위에서 `charness update` 후 이 skill이 언제 보이는지와, 실험 종료 뒤 제거 commit이 `charness update`만으로 언제 사라지는지를 같은 방식으로 기록하면 된다.
