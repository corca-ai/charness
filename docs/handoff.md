...
1. pre-`cautilus`로 계속 가면 `gather` exemplar와 `find-skills` discovery payload를 바탕으로 `access_modes` 다음 단계 capability metadata를 실제 manifest/support/adapter metadata에서 어디까지 고정할지 좁힌다.
