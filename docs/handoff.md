...
- Codex는 managed `~/.agents/plugins/marketplace.json`를 source로 쓰되, operator-facing plugin root는 `~/.codex/plugins/charness`로 둔다. marketplace policy는 `AVAILABLE`로 두고, install/discovery/cache refresh와 visible skill update propagation proof는 끝났다. `tests/charness_cli/test_update_propagation.py`는 standing gate가 아니라 on-demand regression fixture로만 유지한다.
...
- `#18` follow-up도 landed 상태다. `find-skills`는 이제 synced support skill `skills/support/generated/*/SKILL.md`를 별도 layer로 보이고, external integration entry에도 `support_skill_path`, `discovery_stub_path`, `intent_triggers`를 cross-link한다.
- `charness tool sync-support`는 이제 repo-local `.agents/charness-discovery/<tool-id>.md` stub을 남긴다. cold-start agent가 host repo 안에서 grep 가능한 얇은 pointer를 보게 하려는 용도고, `tool doctor` / `tool install` / `tool sync-support`의 next step도 그 stub과 synced support skill path를 같이 말한다.
- `cautilus` manifest에는 verification/evaluation/review intent trigger가 들어갔다. 목적은 generic unit-test intent를 먹는 게 아니라, bounded behavior review/evaluator intent가 있을 때 synced support surface를 더 빨리 발견시키는 것이다.
- `tool doctor` payload는 이제 wrappers가 human `NEXT` parsing 없이 advisory-vs-blocking을 읽을 수 있게 `doctor_disposition`을 가진다. `cautilus` install metadata는 optional host-repo follow-up command도 구조적으로 싣고, `tool doctor` / `tool install` / `tool sync-support`는 binary 다음에 `cautilus install --repo-root <repo>`를 언제 쓸 수 있는지 같이 말한다.
- `retro`는 이제 fresh-eye reader가 present invariant를 absent로 오독한 경우도 auto-retro trigger로 본다. `quality`는 같은 패턴을 declaration/gating quality gap으로 취급하고, adapter-owned `prompt_asset_roots` / `prompt_asset_policy`와 `find_inline_prompt_bulk.py` helper로 inline prompt/content bulk를 advisory inventory로 surface할 수 있다.
- `retro`는 이제 [`probe_host_logs.py`](../skills/public/retro/scripts/probe_host_logs.py)로 Claude/Codex local log availability를 honest하게 probe할 수 있고, [`refresh_recent_lessons.py`](../skills/public/retro/scripts/refresh_recent_lessons.py)가 latest durable retro markdown에서 [`skill-outputs/retro/recent-lessons.md`](../skill-outputs/retro/recent-lessons.md)를 deterministic하게 갱신한다.
- `init-repo`는 이제 [`seed_retro_memory.py`](../skills/public/init-repo/scripts/seed_retro_memory.py)로 `.agents/retro-adapter.yaml`과 `skill-outputs/retro/recent-lessons.md`를 seed할 수 있고, retro memory seam을 켠 repo에서는 `AGENTS.md` memory surface에도 그 digest를 올리라고 가이드한다.
- hidden support source-of-truth도 넓어졌다. `skills/support/specdown/`는 upstream `../specdown` authored skill/docs에서 가져왔고, `skills/support/agent-browser/`는 current packaged support copy를 charness authoritative tree로 승격했다. checked-in plugin export smoke도 두 support skill이 실제로 host bundle에 실리는지 검증한다.
- `narrative`, `find-skills`, `announcement` bootstrap도 더 직접적이 됐다. richer truth-surface fallback 후보를 늘렸고, `resolve_adapter.py` payload와 bootstrap warnings가 이제 artifact path, first-run value, and does-not-do boundary를 같이 말한다. `announcement`는 draft-only default를 숨기지 않고, `find-skills`는 local-first boundary를, `narrative`는 adapter pin 필요성을 더 직접 말한다.
...
0. support-tool follow-up을 이어갈 때는 [docs/support-tool-followup.md](support-tool-followup.md)를 먼저 읽고, 여기서 정한 issue triage와 dogfood acceptance를 그대로 따른다. 특히 `create-cli`, `quality`, `find-skills`/recommendation flow 변경은 모두 skill 수정 + `charness` repo dogfood까지 한 slice로 묶는다.
   - `quality` `#10`, `#13`, `#14`, `#6`, `quality` `#17`, recommendation/install payload의 `quality` 재사용, `init-repo` `#11`, `impl` `#8`, `#7`, `handoff` `#15`, `retro` `#16`, verifier-selection helper, `#18`, support skill vendoring `#20`은 landed 상태다.
1. retro self-improvement follow-up을 이어간다면, 먼저 [`docs/retro-self-improvement-spec.md`](retro-self-improvement-spec.md)를 읽고 다음 slice를 `quality` skill-ergonomics explicit lens 또는 thin retro orchestration helper 중 하나로 고른다.
2. support-tool dogfood를 이어간다면 새 `charness tool install/update/doctor` surface를 실제 다른 머신 또는 temp-home workflow로 한 번 더 점검한다. 특히 provenance-aware route가 있는 tool(`gws-cli`, brew-installed `specdown`, brew-installed `gh`)과 manual-only tool(`cautilus`, release-installed `specdown`)에서 남는 lock/install guidance shape를 본다.
   - temp-home 자동 proof는 이제 installed CLI 기준 `cautilus` support sync + doctor ok, `find-skills` synced-support cross-link, repo-local discovery stub, optional host-repo follow-up command, advisory `doctor_disposition`까지 포함한다. 남은 인간 proof는 실제 다른 머신에서 real `cautilus` binary install/update 경로와, 짧은 agent prompt가 `doctor`/install docs/stub guidance만으로 올바른 다음 행동을 고르는지 보는 것이다.
...
5. 추가 dogfood 결과를 바로 이 handoff나 별도 durable artifact에 남긴다.

## Discuss

- Codex interactive update propagation proof는 끝났다. 남은 host-side concern은 propagation 그 자체가 아니라 reset hygiene와 future on-demand regression 운영이다.
...
- thin CLI는 들어갔고 managed path는 하나로 정리됐다. 남은 질문은 standalone binary distribution UX와 extra host hygiene/reset proof다.
