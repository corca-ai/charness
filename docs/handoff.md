...
- The newest benchmark slice now favors `quality`, `init-repo`, `handoff`, `find-skills`, and `retro`. Refreshing the reviewed `quality` proof-layering seam pulled it back into the top-five cap and pushed `premortem` below the line; `gather` remains below the cap alongside `premortem`, `hitl`, `debug`, `release`, and `spec`. For working-tree diffs, do not rely on `--baseline-ref` shorthand alone: prepare an explicit baseline repo and pass `--baseline-repo <baseline> --candidate-repo <current repo>` to [scripts/eval_cautilus_chatbot_compare.py](../scripts/eval_cautilus_chatbot_compare.py), otherwise uncommitted candidate changes are invisible.
...
4. 다음 pickup의 first move는 `quality`가 top-five로 복귀했다는 전제에서, 남은 omitted 후보 중 `gather-official-path-before-browser-followup`를 다시 올릴지 아니면 `premortem-canonical-subagent-followup`를 다시 당길지 고르는 것이다.
