...
- [Observability contract](../charness-artifacts/spec/2026-08-13-lesson-evaluation-observability-contract.md)
  — owns session open/dispose/reconcile and the DEFERRED decision on auto-opening
  a session from the host `SessionStart` hook.
- [Session retro](../charness-artifacts/retro/2026-08-14-lesson-loop-625-627-626.md)
...
- [#628](https://github.com/corca-ai/charness/issues/628) — owns the scaffold
  write-path class. A third instance was observed live this session in the RETRO
  family; see the retro's Sibling Search.
...
- The lesson loop closed end to end for the first time: a lesson entered the
  ledger by command, was presented, was solicited, and carries an anchored score.
  Regenerate the state with
  `python3 scripts/check_lesson_evaluation_continuity.py --repo-root .` and
...
- [Recent lessons](../charness-artifacts/retro/recent-lessons.md) is the
  generated digest and the ledger is the scored selection surface; they are not
  substitutes, and the digest carries no score, lifecycle, or graduation state.
  Regenerate the selection with
...
- The two failures inherited from the previous slice's skipped broad gate are
  repaired, so the broad gate including `release_only` now passes; the
  [session retro](../charness-artifacts/retro/2026-08-14-lesson-loop-625-627-626.md)
...
- [#618](https://github.com/corca-ai/charness/issues/618),
  ...
  [#624](https://github.com/corca-ai/charness/issues/624),
  [#619](https://github.com/corca-ai/charness/issues/619) — fixes landed in the
  previous slice; closeout still not run.
...
1. Run the `issue` closeout floor for the #625/#627/#626 cohort and the older
   #618-#624 cohort, then post closeout comments. Closing is standing-approved
   only after that floor.
2. Take the release-readiness decision below to `quality` before requesting any
   grant; [#608](https://github.com/corca-ai/charness/issues/608) is the blocking
   half.
3. [#628](https://github.com/corca-ai/charness/issues/628) is independent and
   now has a second family's evidence.
