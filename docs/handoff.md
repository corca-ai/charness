...
- **The `#514/#515/#518` freeze decision is CLOSED.** The operator ruled that a locator changed by an unrelated slice may be re-stamped without re-inspection; four of twenty were re-stamped via `validate_issue_source_freeze.py refreeze`, `validate` passes, and the affected tests are green. Each re-stamped locator carries a note recording that no re-inspection happened and what is not claimed. **Standing residue:** the receipt cannot tell a meaningful change from an incidental one, which is why this needed a human. If a locator's SEMANTICS change, it still needs a real re-read — the policy does not cover that.
