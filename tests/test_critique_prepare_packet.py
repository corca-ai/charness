...
def test_changed_path_mode_ignores_markdown_worker_evidence(tmp_path: Path) -> None:
    worker_record = tmp_path / "charness-artifacts/critique/workers/run/review-prompt.md"
    worker_record.parent.mkdir(parents=True, exist_ok=True)
    worker_record.write_text("worker prompt\n", encoding="utf-8")

    assert critique_candidate_paths(
        tmp_path,
        ["charness-artifacts/critique/workers/run/review-prompt.md"],
        all_artifacts=False,
    ) == []
