...
def test_validate_quality_artifact_rejects_executed_that_states_no_review_ran(tmp_path: Path) -> None:
    """The stub the sweep reproduced: `executed` plus a confession that nothing ran."""
    repo = seed_repo(
        tmp_path,
        artifact_with_delegated_review("- status: executed (no reviewer, no findings, nothing ran)."),
    )
...
def test_validate_quality_artifact_reads_the_declared_delegated_review_status(tmp_path: Path) -> None:
...
def test_validate_quality_artifact_keeps_the_boundary_fingerprint_phrasing_legal(tmp_path: Path) -> None:
...
def test_validate_quality_artifact_ignores_fill_guard_comments_in_delegated_review(tmp_path: Path) -> None:
...
def test_validate_quality_artifact_accepts_a_cited_review_record_as_substantiation(tmp_path: Path) -> None:
...
def test_validate_quality_artifact_rejects_a_modified_reviewer_denial(tmp_path: Path) -> None:
    ...
    repo = seed_repo(
        tmp_path,
        artifact_with_delegated_review("- status: executed (no bounded reviewer ran, nothing found)."),
    )
    with pytest.raises(ValidationError, match=re.escape("must not also state that no review ran")):
        validate(repo)
