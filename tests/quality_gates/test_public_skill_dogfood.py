...
def test_public_skill_dogfood_matrix_reports_prompt_artifact_and_evidence() -> None:
    ...
    prompt = achieve["prompt"].lower()
    assert "fresh agent" in prompt
    assert not any(term in prompt for term in ("ideation", "spec", "probe", "assumption"))
    evidence = "\n".join(achieve["acceptance_evidence"])
    assert "different next moves for pass and fail" in evidence
    assert "direct deterministic path for local structural claims" in evidence
