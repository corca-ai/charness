...
def _record(payload: dict, prepared_commit: str, prepared_record: str,
            narrative_path: str = NARRATIVE_PATH) -> str:
    return json.dumps(claims_review_record(
...
def _write_narrative(repo: Path, prepared_commit: str, payload: dict,
                     narrative_path: str = NARRATIVE_PATH) -> str:
...
def _source_bound_evidence(tmp_path: Path):
    ...
    path = commit_claims_review(
        ...
        target_version=payload["target_version"], tag_name=payload["tag_name"], stem="source-claims",
    )
...
def _distinctness_invoke(tmp_path: Path):
    ...
    prepared = {"commit": "prepared0123456789", "path": "charness-artifacts/release/latest.md",
                "sha256": "record-sha"}
    record_path = "charness-artifacts/release-review/review.json"
    narrative_path = "charness-artifacts/release-review/review.md"
    parents = ("git", "show", "-s", "--format=%P", "evidence")
    diff = ("git", "diff-tree", "--no-commit-id", "--name-only", "-r", prepared["commit"], "evidence")
    status = ("git", "diff-tree", "--no-commit-id", "--name-status", "-r", prepared["commit"], "evidence")

    def invoke(record: dict, *, changed: list[str] | None = None, narrative: str | None = "default",
        ...
        responses = {
            ...
            diff: (0, "".join(f"{line}\n" for line in changed)),
            status: (0, "".join(
                f"{narrative_status if line.endswith('.md') else 'M'}\t{line}\n" for line in changed
            )),
        ...
        return CLAIMS_REVIEW.validate_claims_review(
            tmp_path, prepared=prepared, evidence_commit="evidence", artifact_path=record_path,
            target_version="1.2.3", tag_name="v1.2.3", run=run,
        )

    def record(**overrides) -> dict:
        base = claims_review_record(
            prepared_commit=prepared["commit"], prepared_record="", target_version="1.2.3",
            tag_name="v1.2.3", narrative_path=narrative_path,
        )
...
def test_distinctness_must_be_declared_not_inferred(tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit, match="requires an `observer_distinctness` object"):
        invoke(missing)
    with pytest.raises(SystemExit, match="must name the concrete signal"):
        invoke(record(observer_distinctness={"kind": "separate-host", "signal": "  ",
...
def test_a_same_agent_reread_has_no_passing_kind(tmp_path: Path) -> None:
    ...
    for kind in ("same-agent", "self", "unproven"):
        with pytest.raises(SystemExit, match="must be one of"):
            invoke(record(observer_distinctness={"kind": kind, "signal": "same session reread",
...
def test_an_unproven_verdict_is_accepted_with_its_concrete_signal(tmp_path: Path) -> None:
    ...
    result = invoke(
        record(verdict="unproven",
               observer_distinctness={"kind": "unproven",
    ...
    with pytest.raises(SystemExit, match="requires `observer_distinctness.kind"):
        invoke(record(verdict="unproven",
                      observer_distinctness={"kind": "separate-host", "signal": "s",
                                             "review_artifact": None}),
               changed=["charness-artifacts/release-review/review.json"], narrative=None)
...
def test_the_evidence_commit_may_carry_only_the_named_review_files(tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit, match="and the review_artifact it names"):
        invoke(record(), changed=["charness-artifacts/release-review/review.json",
...
@pytest.mark.release_only
def test_a_second_prepare_over_an_outstanding_marker_cannot_publish_unreviewed(tmp_path: Path) -> None:
