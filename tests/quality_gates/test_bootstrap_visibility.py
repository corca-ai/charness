...
def test_narrative_resolve_adapter_fallback_prefers_richer_truth_docs(tmp_path: Path) -> None:
    ...
    assert "skill-outputs/narrative/narrative.md" in payload["warnings"][1]
...
def test_find_skills_resolve_adapter_explains_local_first_boundary(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["artifact_path"] == "skill-outputs/find-skills/find-skills.md"
...
def test_announcement_resolve_adapter_explains_draft_only_defaults(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["artifact_path"] == "skill-outputs/announcement/announcement.md"
    assert payload["record_path"] == "skill-outputs/announcement/announcements.jsonl"
