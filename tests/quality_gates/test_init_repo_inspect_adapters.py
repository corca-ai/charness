...
def test_init_repo_inspect_recommends_seed_worktree_adapter_for_lefthook(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
...
def test_init_repo_inspect_recommends_seed_worktree_adapter_for_husky(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_minimal_repo_with_adapter(repo)
...
def test_init_repo_inspect_recommends_seed_worktree_adapter_for_simple_git_hooks(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_minimal_repo_with_adapter(repo)
...
def test_init_repo_inspect_skips_worktree_adapter_recommendation_when_present(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_minimal_repo_with_adapter(repo)
    (repo / "lefthook.yml").write_text("pre-commit:\n  commands: {}\n", encoding="utf-8")
...
def test_init_repo_inspect_skips_worktree_adapter_recommendation_when_no_hook_manager(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_minimal_repo_with_adapter(repo)
...
def test_init_repo_inspect_promotes_init_repo_adapter_absence_to_recommendation(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_normalize_repo(repo, "# Agents\n")

    payload = _run_inspect(repo)

    init_state = payload["agent_docs"]["normalization"]["init_repo_adapter"]
    assert init_state["adapter_exists"] is False
    assert init_state["adapter_path"] is None

    rec_index = {item["id"]: item for item in payload["recommendations"]}
    assert "init_repo_adapter_missing" in rec_index
    rec = rec_index["init_repo_adapter_missing"]
    assert rec["target"] == ".agents/init-repo-adapter.yaml"
    assert rec["priority"] == "advisory"
    assert rec["kind"] == "seed_artifact"
    assert "preset provenance" in rec["suggested_action"]


def test_init_repo_inspect_omits_init_repo_adapter_recommendation_when_present(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _seed_minimal_repo_with_adapter(repo)
    ...
    rec_ids = [item["id"] for item in payload["recommendations"]]
    assert "init_repo_adapter_missing" not in rec_ids
