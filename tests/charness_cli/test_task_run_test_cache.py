...
def test_import_closure_change_invalidates_cached_test(tmp_path: Path, monkeypatch) -> None:
    ...
    test_file = repo / "tests" / "test_sample.py"
    test_file.parent.mkdir(parents=True)
    test_file.write_text("from scripts.support import VALUE\n", encoding="utf-8")
    support = repo / "scripts" / "support.py"
    support.parent.mkdir(parents=True)
...
@pytest.mark.parametrize(
    ("field", "value"),
    [("result_kind", "failed"), ("exit_code", 1), ("verify_status", "fail")],
)
def test_skip_requires_frozen_success_envelope(
    ...
    if field == "verify_status":
        record["verify_outcome"]["status"] = value
    else:
        record[field] = value
