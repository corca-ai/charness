...
def _validate_receipt(receipt: dict[str, Any]) -> tuple[bool, str]:
...
def build_report(
    *,
    ...
    receipt_ok, receipt_reason = _validate_receipt(receipt)
    provenance = {
        "scope": scope,
    ...
    ledger_ok = attempt.state == FINDINGS_RECEIVED and attempt.approval_eligible
    ...
    if not receipt_ok:
        ...
        reason = f"delivery ledger state is {attempt.state!r}, not findings-received"
    else:
