...
try:
    from reviewer_delivery import _read, _write
    from reviewer_output import emit_yaml
    from reviewer_worker_report import ReportError, build_report
except ImportError:
    from skills.shared.scripts.reviewer_delivery import _read, _write
...
RESOLVE_ADAPTER = ROOT / "skills/public/critique/scripts/resolve_adapter.py"
DEFAULT_SCHEMA = ROOT / "skills/shared/references/bounded-review-result.schema.json"
...
def _adapter(repo_root: Path) -> dict[str, Any]:
    result = subprocess.run(
        [sys.executable, str(RESOLVE_ADAPTER), "--repo-root", str(repo_root)],
        cwd=repo_root,
        capture_output=True,
        text=True,
        check=False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        mode = args.execution_mode or runner.get("mode", "file-backed-worker")
        backend = args.backend or runner.get("backend")
        timeout = args.timeout_seconds or runner.get("timeout_seconds", 900)
        ...
        if args.stdout_file:
            worker_command.extend(["--stdout-file", str(args.stdout_file.resolve())])
        if args.stderr_file:
            worker_command.extend(["--stderr-file", str(args.stderr_file.resolve())])
        if args.timeout_seconds is not None or runner.get("timeout_seconds") is not None:
        ...
        receipt = json.loads(args.receipt_file.resolve().read_text(encoding="utf-8"))
        ledger = _read(ledger_path)
