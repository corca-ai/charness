...
def main() -> int:
    ...
    parser.add_argument("--url", required=True, help="Public URL to acquire.")
    parser.add_argument("--repo-root", type=Path, default=Path.cwd(), help="Repository root for route and capability resolution.")
    parser.add_argument("--intent", choices=("single", "collect"), default="single", help="Acquisition intent: one source or collect-mode fallback stages, including network-recon when supported.")
    parser.add_argument("--browser-mode", choices=("auto", "off", "always"), default="auto", help="Browser-capable fallback policy: auto, off, or always; always still tries browser fallbacks when they are needed.")
    parser.add_argument("--timeout", type=int, default=20, help="Per-command timeout in seconds.")
    parser.add_argument("--direct-response-file", type=Path, help="Read a seeded direct response from this file.")
    ...
    parser.add_argument("--include-selected-content", action="store_true", help="Include a selected content excerpt in the output when a selected attempt succeeds.")
    parser.add_argument("--selected-content-max-chars", type=_positive_int, default=200_000, help="Maximum characters in the selected content excerpt.")
