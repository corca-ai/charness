...
1. Create a new `.spec.md` file. One file per feature ([best practice](best-practices.md#keep-documents-focused)).
...
4. Link the new file from the index: `- `Feature Name``.
...
If the project uses [traceability](traceability.md), add the appropriate trace links (e.g., ``covers::Feature`` in the parent goal document).
...
See [Doctest Blocks](syntax.md#doctest-blocks) for full syntax.
...
This renders as a green/red badge in the HTML report without interrupting the prose flow. See [Inline expect](syntax.md#inline-expect).

### Add check tables for repeated patterns

If you see three or more `run:shell` blocks testing variations of the same thing, refactor into a [check table](syntax.md#check-tables):
...
This requires an [adapter check](adapter-protocol.md), but the spec becomes pure data.
...
When the state space is too large for examples, add an [Alloy model](alloy.md) in the same section as the implementation checks. Use models for:
...
See [Alloy patterns](best-practices.md#patterns) for worked examples.

### Add traceability

If the project has goals or feature documents, add [trace edges](traceability.md) to ensure coverage. Run `specdown trace -strict` to find gaps.
...
2. **Determine the cause**:
   ...
   - **Environment issue** — missing dependency, stale state, ordering problem. Fix setup/teardown or add [hooks](syntax.md#setup-and-teardown-hooks).
...
Only one filter can be active at a time. See [Filter](cli.md#filter).
...
- **Add summary lines** to long shell blocks so the report stays scannable. See [Summary Lines](syntax.md#summary-lines).
