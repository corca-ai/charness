...
```markdown
This epic `covers::Login Story` and
...
The `trace` key in `<repo-root>/specdown.json` configures traceability:
...
No `trace` config in `<repo-root>/specdown.json` means everything works as before.
