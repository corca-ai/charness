...
def preset_contract(
    ...
    try:
        ...
        return {"state": "unavailable", "reason": f"could not resolve {path.relative_to(repo_root)}"}
    if not resolved_path.is_relative_to(canonical_repo_root / PRESETS_DIR):
        return {"state": "unavailable", "reason": f"{path.relative_to(repo_root)} must resolve inside presets/"}
    try:
        data = validator.validate_preset(path)
    except (OSError, ValueError, TypeError, validator.ValidationError) as exc:
        return {"state": "unavailable", "reason": f"{path.relative_to(repo_root)} is not a valid preset: {exc}"}
    reconciliation = data.get("reconciliation") if isinstance(data, dict) else None
    if reconciliation is None:
        return {"state": "metadata-only", "reason": "preset declares no reconciliation prescription"}
    ...
    if not isinstance(required, list) or not required or not all(isinstance(item, str) and item.strip() for item in required):
        return {"state": "unavailable", "reason": "reconciliation.required_adapter_commands must be a non-empty string list"}
...
def preset_rows(
    ...
    for preset in raw.get("preset_lineage") if isinstance(raw.get("preset_lineage"), list) else []:
        ...
        if contract["state"] == "metadata-only":
            row.update({"reconciliation_state": "metadata-only", "reconciliation_reason": contract["reason"]})
        elif contract["state"] == "unavailable":
            row.update({"reconciliation_state": "unavailable", "reconciliation_reason": contract["reason"]})
            gaps.append({"kind": "preset_reconciliation_unavailable", "detail": f"{preset}: {contract['reason']}"})
        else:
            required = contract["required_adapter_commands"]
            missing = [command for command in required if command not in declared_commands]
            row.update({"required_adapter_commands": required, "missing_adapter_commands": missing, "reconciliation_state": "missing" if missing else "reconciled"})
            gaps.extend({"kind": "preset_requirement_missing", "detail": f"{preset}: declare adapter command: {command}"} for command in missing)
        rows.append(row)
    return rows, gaps
