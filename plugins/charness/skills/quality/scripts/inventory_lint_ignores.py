...
def _recommendation(*, tool: str, file_level: bool, blanket: bool) -> str:
    if blanket:
        return (
            "Blanket lint suppression is a strong smell; prefer a structural fix or a rule-specific suppression with a clear reason."
        )
    if file_level and tool == "ruff":
        return (
            "File-level Ruff suppression deserves explicit review; prefer packaging or launcher structure that makes the import order legal."
        )
    if file_level:
        ...
        for line_no, line in enumerate(text.splitlines(), start=1):
            ...
            for match in PYTHON_PYLINT_RE.finditer(line):
                _record_finding(
                    ...
                    line_no=line_no,
                    tool="pylint",
                    scope="file" if line.lstrip().startswith("#") else "inline",
                    codes=_parse_codes(match.group("codes")),
                    raw=line,
                )
