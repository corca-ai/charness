...
2. Detect the current gate surface.
   ...
   - when advisory inventories, helper scripts, or dead-file review are in scope, run `$SKILL_DIR/scripts/inventory_gitignore_scan_hygiene.py`; use `vulture` for Python and `knip` for JavaScript/TypeScript instead of narrowing stale-file risk to migrations
