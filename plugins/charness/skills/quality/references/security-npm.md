...
`<authoring-repo>/scripts/check_supply_chain.py` owns those checks for the current `charness`
bar.
...
- run npm's advisory flow when dependency changes matter enough to justify a
  ...
  `<authoring-repo>/scripts/check_supply_chain_online.py` with `npm audit --json`
