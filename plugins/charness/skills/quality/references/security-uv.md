...
`<authoring-repo>/scripts/check_supply_chain.py` currently owns this offline alignment check for
`charness`.
...
- if a downstream repo wants a standing online audit command, make it explicit
  ...
  `<authoring-repo>/scripts/check_supply_chain_online.py` with `uv audit --frozen`
