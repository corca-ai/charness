...
Behavior-test recommendations follow the boundary in `behavior-testing.md`.
They remain recommend-only unless the consumer supplies an explicit source
packet and an existing evaluator or bounded review path; do not install a
Charness-owned runner for this class of proof.
