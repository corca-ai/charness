...
def infer_cautilus_defaults(repo_root: Path, *, run_mode: str = "ask") -> dict[str, Any]:
    return {
        ...
        "eval_test_command_templates": [
            "node ./scripts/agent-runtime/run-local-eval-test.mjs --repo-root . --workspace {candidate_repo} --cases-file {eval_cases_file} --output-file {eval_observed_file} --artifact-dir {output_dir}/eval-test --backend {backend} --sandbox read-only --timeout-ms 180000 --codex-model gpt-5.4-mini --codex-reasoning-effort low --claude-permission-mode dontAsk"
        ],
        "held_out_command_templates": [
