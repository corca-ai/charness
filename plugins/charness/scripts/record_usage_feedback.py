...
def main() -> int:
    ...
    if replay is not None:
        if {key: replay[key] for key in record if key != "timestamp"} == {key: record[key] for key in record if key != "timestamp"}:
            _print({"status": "replay_noop", "executed": False, "feedback_id": feedback_id, "records_path": _portable_path(repo_root, records_path), "errors": []}, args.json)
            return 0
        _print({"status": "conflicting_feedback_id", "executed": False, "feedback_id": feedback_id, "errors": errors}, args.json)
        return 2
    if errors:
        _print({"status": "invalid_feedback", "executed": False, "feedback_id": feedback_id, "errors": errors}, args.json)
        return 2
